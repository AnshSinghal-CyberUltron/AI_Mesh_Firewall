/**
 * Port of frontend/src/utils/routingExplain.js (demo vanilla copy).
 */
import { formatRoutingReason, isRoutingReroute } from "./zeroshieldBrand.js";
...
function buildBecauseClause({ tops, sensitivity, inactiveRemap, sensFallback, scoreTie }) {
  ...
  if (tops.length > 0) parts.push(`it ranked best on ${tops.join(" and ")}`);
  if (scoreTie) {
    parts.push("top candidates tied on score (list-order / adjudicator broke the tie)");
  }
  if (inactiveRemap) parts.push("your requested model is not in the active router pool");
