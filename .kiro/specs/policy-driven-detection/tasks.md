...
- [ ] 4. Tier-2 opt-in gating on the chat + OpenAI-SDK path
  ...
  - [-] 4.2 Write property test for Tier-2 opt-in gating
  ...
  - [-] 4.3 Write property test for Tier-2/Tier-1 separation
  ...
  - [-] 4.4 Write test for OpenAI-SDK surface Tier-2 parity
...
- [ ] 5. Re-home the built-in families as seeded, default-OFF policy packages (control plane)
  - [-] 5.1 Add the built-in-family catalog module
  ...
  - [~] 5.2 Add the idempotent, default-OFF seeder
  ...
  - [~] 5.3 Write unit tests for seeding and default-OFF
  ...
  - [~] 5.4 Write test for enable/disable a package end to end (control → compiled bundle)
  ...
  - [~] 5.5 Write property test for per-package isolation
...
- [ ] 6. Remove the legacy scan toggles as detection drivers
  ...
  - [~] 6.2 Remove the toggles from the control-plane config serialization + frontend
  ...
  - [-] 6.3 Write property test for legacy-toggle inertness
...
- [ ] 7. Extend the policy-driven model to every non-chat surface
  - [~] 7.1 Streaming output guard
  ...
  - [~] 7.5 Write property tests for fail-toward-no-detection and surface coherence
...
- [ ] 8. Frontend Tier-2 enable/disable control
  - [~] 8.1 Add the Tier-2 toggle to the firewall settings UI
  ...
  - [~] 8.2 Write a frontend test for the toggle reflecting + changing backend state
