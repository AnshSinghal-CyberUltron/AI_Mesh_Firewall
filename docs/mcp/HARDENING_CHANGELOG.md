...
- **RESIDUAL:** the internal streamable-http path's `_scan_internal_result` scans only `result.content` (not a bare error frame / `structuredContent`) — a narrower residual than the sandbox complete-skip fixed here; noted for a follow-up.
