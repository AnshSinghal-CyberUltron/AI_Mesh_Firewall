...
mk() { # name type ip kind
  ...
  if [[ $? == 0 ]]; then
    python3 $H/deploy/ledger.py $EVID/vm-ledger.jsonl create $name $Z $mt pbf "re-created for 1-unit load-rule repeat (same IP $ip)"
    echo "$Z" > $EVID/.zone-$name; echo "CREATED $name $ip"
  else
    echo "FAILED $name: $(echo "$out" | tail -2 | tr '\n' ' ' | cut -c1-300)"
  fi
}
# minimal 1-unit setup: unit-2, edge, Redis, 3 loadgens, provider-1 (same IPs as the main lane)
mk rv-pbf-unit-2 g2-standard-24 10.146.0.2 g2 & mk rv-pbf-edge-1 c4-highcpu-16 10.146.0.10 cpu &
mk rv-pbf-redis-1 c4-standard-8 10.146.0.13 cpu & mk rv-pbf-prov-1 c4-highcpu-8 10.146.0.8 cpu &
