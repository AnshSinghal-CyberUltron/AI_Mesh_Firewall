#!/usr/bin/env python3
"""proto-bench-fleet copy of reviewer-observability/c4_client.py with one change: --all disables the
fnv1a64(rid) % 10 filter (runs use olg/synthprov -sample-mod 1, so every stream carries per-chunk data).
        ...
        with R.open_any(f) as fh:
            for line in fh:
                ...
                if c.get("ph") != 2:
                    continue
                rid = c["rid"]
                if not ALL and R.fnv1a64(rid) % 10 != 0:
                    continue
                p = by_rid.get(rid)
