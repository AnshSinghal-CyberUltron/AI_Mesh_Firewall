...
if [[ $MODE == sut ]]; then
  (cd "$EVID/scripts" && $PY c4_client_all.py "$RUNS/$run" --all > "$RUNS/$run/c4.json" 2> "$RUNS/$run/c4.err") || true
  $PY "$EVID/scripts/c4_rule.py" "$RUNS/$run" >> "$RUNS/$run/pbf.stdout" 2>&1 || true
fi
mkdir -p "$EVID/runs/$run"
cp "$RUNS/$run/step.json" "$RUNS/$run"/pbf_summary.* "$RUNS/$run"/c4.json "$RUNS/$run"/c4_rule.json "$EVID/runs/$run/" 2>/dev/null || true
