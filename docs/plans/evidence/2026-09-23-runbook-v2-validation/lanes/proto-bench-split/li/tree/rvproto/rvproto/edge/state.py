...
from rvproto.detect.guard.factory import build
...
if TYPE_CHECKING:
    from rvproto.detect.guard.factory import GuardImpl
...
class State:
    ...
    def inspector(self, plan: object) -> PlanInspector:
        ...
        if ins is None:
            cls = PlanInspector if self.s.exp_output_scan else NoScanInspector  # experiment flag
            ins = self.inspectors[key] = cls(self.matcher, self.det, plan)  # type: ignore[arg-type]
        return ins
