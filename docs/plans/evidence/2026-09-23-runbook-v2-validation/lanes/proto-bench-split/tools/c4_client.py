...
def main():
    ...
    global SAMPLE_MOD  # split lane: --sample-mod N (default 10 = the reviewer's uniform 10% sample; 1 = every stream)
    SAMPLE_MOD = next((int(x.split("=", 1)[1]) for x in sys.argv if x.startswith("--sample-mod=")), 10)
    ...
    for d in sorted(p for p in root.glob(lgg) if p.is_dir()):
        ...
        with R.open_any(f) as fh:
            for line in fh:
                ...
                lagmax.append(lm)
                cnt["sse"] += 1
    out = {"run": str(root), "sample_mod": SAMPLE_MOD, "counts": dict(cnt),
