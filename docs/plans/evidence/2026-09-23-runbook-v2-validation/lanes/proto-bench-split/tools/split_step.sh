...
#     BUILD (recorded in step.json; default rvproto-frozen-1)  SAMPLER_EXTRA (e.g. --sigusr1 for the li-knobs build,
#     whose workers/owners dump metrics only on SIGUSR1 when RV_METRICS_DUMP_S=0)
#   Also per step (background): proto-bench-fleet scripts/c4_client_all.py --all -> c4_all.json (controller's C4 tool)
set -euo pipefail
...
{"run": "$run", "rate": $rate, "mode": "$MODE", "corpus": "$(basename "$corpus")", "targets": "$TARGETS",
 ...
 "build": "$BUILD", "sampler_extra": "$SAMPLER_EXTRA",
...
for v in $sut; do
  ...
  rssh "$v" "mkdir -p /dev/shm/split/$run/sut && cd /dev/shm/split/$run/sut && (curl -s -m 5 localhost:8400/_rv/contract > contract.json || true); \
    ...
    --start-at $start --ramp $RAMP --warmup $WARMUP --duration $DURATION --tail 40 $SAMPLER_EXTRA > sampler.log 2>&1 < /dev/null & echo \$! > sampler.pid"
done
i=0
for n in $LGS; do
...
analyze_all() {  # verdict inputs first (the ladder driver waits): split_metrics + C4 in parallel; the rest in background
  ...
  (
    ...
    $PY "$SP/evidence/proto-bench-fleet/scripts/c4_client_all.py" "$RUNS/$run" --all $d > "$RUNS/$run/c4_all.json" 2> "$RUNS/$run/c4_all.stderr" || true
    cp -r "$RUNS/$run/analysis" "$EVID/runs/$run/harness-analysis" 2>/dev/null || true
    cp "$RUNS/$run"/split_sut.json "$RUNS/$run"/c4_all.json "$RUNS/$run"/*.stdout "$EVID/runs/$run/" 2>/dev/null || true
