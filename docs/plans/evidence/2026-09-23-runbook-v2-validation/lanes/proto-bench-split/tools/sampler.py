...
(split lane copy of proto-bench-unit tools/unit_sampler.py, current version with --sigusr1; only change: records every non-lo NIC.)
...
def conns_by_pid(port: int = 8400) -> dict:
    """Established client connections per serving process (ss -tnp on the listening port)."""
    try:
        txt = subprocess.run(["ss", "-tnpH", "state", "established", f"( sport = :{port} )"], capture_output=True,
...
def signal_dump() -> None:
    """li-tree processes dump their metrics on SIGUSR1 (only sent when --sigusr1 is given)."""
    for pid, role in procs().items():
        if role == "worker" or role.startswith("owner"):
            try:
...
def main() -> int:
    ...
    ap.add_argument("--sigusr1", action="store_true", help="send SIGUSR1 (li-tree on-demand dump) before snapshots")
    a = ap.parse_args()
    METRICS = Path(a.metrics_dir)
    SIGUSR1_SNAP = a.sigusr1
    out = Path(a.out)
