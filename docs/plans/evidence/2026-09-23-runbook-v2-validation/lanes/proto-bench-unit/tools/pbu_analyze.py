...
# Pinned harness analyzer: the controller-approved analyze.py (harness commit fc5c983, sha256 c9f89fe8...),
# frozen here because SP/harness/analyze.py kept evolving during this lane (f0d7624 changed evaluate()'s
# interface). pinned/shared -> SP/harness/shared (canary values used for output-REDACT length checks).
sys.path.insert(0, str(Path(__file__).resolve().parent / "pinned"))
...
def sut_side(run: Path, n_meas: int) -> dict | None:
    ...
    if not u.exists():
        return None
    if not (u / "schedule.json").exists():
        return {"missing": "unit sampler did not run (see unit/sampler.log)",
                "sampler_log": (u / "sampler.log").read_text()[-400:] if (u / "sampler.log").exists() else None}
    ...
    if len(win) >= 2:
        ...
        out["worker_util"] = {"n": len(wutil), "min": wutil[0] if wutil else None,
                              ...
                              "all_sorted": wutil}
        # established client connections per worker (sampler 'conns' every 10 s; max over the window)
        wpids = {pid for pid, pb in b["procs"].items() if pb["role"] == "worker"}
        cmax: dict = {}
        for smp in win:
            for pid, n in (smp.get("conns") or {}).items():
                if pid in wpids:
                    cmax[pid] = max(cmax.get(pid, 0), n)
        if cmax:
            cv = sorted(cmax.values())
            out["worker_conns_max"] = {"workers_seen": len(cv), "min": cv[0], "median": cv[len(cv) // 2], "max": cv[-1],
                                       "total": sum(cv)}
