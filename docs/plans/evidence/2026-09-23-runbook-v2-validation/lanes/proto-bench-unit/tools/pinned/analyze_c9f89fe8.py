...
Qualified (per request): HTTP 200, complete (SSE [DONE] / valid JSON), no transport/stream error,
  joined to exactly one provider call, provider 200, content sha equal to the provider's (or, for an
  expected output REDACT: the provider really injected, no output canary reached the client, and
  client length == provider length - injected piece + 0..64 replacement bytes), unique nonce;
  in --mode sut additionally disposition ALLOW|REDACT and every --profile-stages stage :E.
Expected policy blocks (correctness traffic) are counted separately and are not errors.
Rate step PASS (mode sut): p99 T_fw_addon < --slo-ms over the qualified cohort, errors <= --err-budget
...
BLOCK_STATUSES = (400, 403, 422, 451)  # policy-block statuses; 401/404/408/413/429/5xx are never policy blocks
...
def evaluate(c: dict, p: dict | None, calls: int, args, exp: dict, stages_req: list[str], dup_nonce: bool):
    """Returns (outcome, reasons, metrics, safety_reasons). outcome in qualified|expected_block|error."""
    reasons: list[str] = []
    safety: list[str] = []
    m: dict = {}
    ...
    if args.policy == "enforce" and e_input == "BLOCK":
        if status in BLOCK_STATUSES and p is None:
            return "expected_block", [], m, safety
        reasons.append("expected_block_not_blocked" if status == 200 else f"block_status_{status}")
        return "error", reasons, m, safety
    ...
    if args.mode == "sut":
        ...
        if disp not in ("ALLOW", "REDACT"):
            reasons.append(f"disposition_{disp or 'missing'}")
        if stages_req:
            got = {}
            for part in (c.get("stages") or "").split(","):
                if ":" in part:
                    k, v = part.split(":", 1)
                    got[k.strip()] = v.strip()
            for s in stages_req:
                if got.get(s) != "E":
                    reasons.append(f"stage_{s}_{got.get(s, 'missing')}")
    if e_input == "REDACT" and args.policy == "enforce" and args.mode == "sut":
        if (c.get("disp") or "").upper() != "REDACT":
            reasons.append("redact_expected_disposition_" + ((c.get("disp") or "missing").upper()))
    # T_fw_addon
    if "addon_total" in m:
        if c.get("stream"):
            parts = [m["addon_total"]]
...
def analyze(args) -> dict:
    ...
    per_sec: dict = defaultdict(lambda: [0, 0, 0])  # sched second -> offered, qualified, errors
    seen_rids = set()
    for f in client_files:
        with open_any(f) as fh:
            for line in fh:
    ...
    if args.mode == "sut":
        checks = {
            ...
            "error_rate_le_budget": err_rate is not None and err_rate <= args.err_budget,
    ...
    summary = {
        ...
        "qualified": outcomes["qualified"], "expected_blocks": outcomes["expected_block"], "errors": errors,
...
def write_md(s: dict, path: Path):
    ...
    L.append(f"- qualified: {s['qualified']} ({s['qualified_rps']} req/s); expected blocks: {s['expected_blocks']}; "
             f"errors: {s['errors']} (rate {s['error_rate']})")
    L.append(f"- error reasons: {s['error_reasons']}")
