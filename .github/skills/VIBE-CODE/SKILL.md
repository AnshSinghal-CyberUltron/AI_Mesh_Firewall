...
### **Step 6 — /verify: Live Local Verification (MANDATORY BEFORE DECLARING ANY FIX DONE)**
...
### **Step 6A — Start All Services + Force Local Execution**

Run ALL services locally to ensure the system is fully operational:

```bash
# Infrastructure
(cd compose && docker compose up -d)
(cd compose/vector-db && docker compose up -d)

# Backend
(cd backend && uv sync && uv run fastapi dev main.py --port 8081)

# Frontend
(cd frontend && npm install && npm run dev)

# Workers
(cd RabbitMQ/worker && uv sync && uv run celery -A scan_worker worker -P threads --loglevel=info -Q scan_worker_queue)
(cd RabbitMQ/worker && uv run celery -A scan_worker worker -P threads --loglevel=info -Q cicd_worker_queue)

(cd RabbitMQ/guardrail_worker && uv sync && uv run celery -A guardrail_worker worker -P prefork --concurrency=4 --loglevel=info -Q guardrail_worker_queue)

(cd RabbitMQ/dast_worker && uv sync && uv run celery -A dast_worker worker -P threads --loglevel=info -Q dast_worker_queue)

# ZAP Scanner
(cd compose/zap && docker compose up -d)

# Monitoring
(cd celery_monitoring && uv sync && uv run flower --broker=amqp://admin:<password>@localhost:4672 --port=5555)
```

---

### **Step 6B — Health + Runtime Verification (MANDATORY)**

After starting services, **verify everything is actually running and connected**:
...
# RabbitMQ queues (verify workers are consuming)
...
### **Step 6C — Functional Validation (Real System Behavior)**
...
# Queue test (send job → verify worker logs)
# Observe logs in worker terminals

# ZAP scan validation (if applicable)
# Ensure ZAP container is active and reachable
```

---

### **Step 6D — Log-Based Verification**

Monitor logs LIVE (critical):
...
# Worker logs
# (observe celery output for task execution)
```
...
Fix: [Title]
─────────────────────────────────────
Services running locally:   [ ] YES / [ ] NO
Docker containers healthy:  [ ] YES / [ ] NO
Workers consuming queues:   [ ] YES / [ ] NO
API responses correct:      [ ] YES / [ ] NO
Logs clean:                 [ ] YES / [ ] NO
