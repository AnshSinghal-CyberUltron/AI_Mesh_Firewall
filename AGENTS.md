...
- PIPELINE-0024 (2026-07-03) — PIPELINE-TRACE VIEW UI (P6-24): LogDetailPage + StageTimeline
  impeccable revamp — CHART_PALETTE (no inline hex), equal-height MetricCards, responsive metrics
  grid, min-h-9/11 touch targets, CollapsibleSection a11y + testIds, routing-decision-card +
  pipeline-trace-view data-testids; StageTimeline responsive popover + sm:grid-cols-2 detail grid.
...
- PIPELINE-0026 (2026-07-03) — CISO 100-RULE PACKAGE DESIGN (P7-26): docs/policies/CISO_100.md
 ...
 - PIPELINE-0029 (2026-07-06) — OUTPUTVERDICT JSON 500 FIX: main.py _strip_internal_completion_keys pops _pipeline_output_scan_verdict before client JSONResponse (proxy_chat+sync_pre_llm); _resolve_pipeline_blocked_by code==output_blocked→output_guardrail; liveGateway output_guardrail_ms metric+honest 500 when guard ran. +2 GW +2 FE tests. Gate: pytest+npm test:unit.
 - PIPELINE-0030 (2026-07-06) — SCAN 296170: output-guard smart-mask noop on reasoning channel (_output_redact_redaction_possible scan_text-first + proxy_chat redaction_possible); pipeline_trace output_guard block operator output_text preview; simulatorRoutingPreferences(org routing_enabled) Attack+Isolation; remove bare 120b canonicalize token; LogDetailPage withheld preview. north-mini=log remapping inactive nemotron (llm_router). +GW/FE/control tests. Gate: rebuild+live repro.
 - PIPELINE-0031 (2026-07-15) — AUTO-ROUTING BIAS FIX (north-mini list-order): sensitivity soft-fallback + honesty factors; ROUTING_ADJUDICATOR_ALWAYS=true (single-candidate + weight≥0.95 skip); differentiate_routing_catalog → Redis; remap prefers highest-scored active + remapped_from; nested weights alias; RoutingDecisionCard/demo transparency. VERIFY: 14 GW routing tests; live adversarial cost→north-mini risk→gpt-5.2, sensitivity ladder 200.
 - PIPELINE-0032 (2026-09-10) — PII PLATFORM FLOOR + 503 TRACE HONESTY: scanner PII/secret/DoS floor restored; resolve_and_enforce gets scanner_* for data-protection threats only; JSON 503 keeps pipeline_trace (input_scan redact, model_output error); FE never badges ALLOW/"No threat detected" on 5xx without a scan. VERIFY: test_pipeline_llm_error_trace + scan-only PII + liveGateway.test.js 23.
 - PIPELINE-0033 (2026-09-17) — TIER-2 INJECTION FLOOR + SCAN DETAIL HONESTY: resolve_and_enforce receives ZeroShield Model prompt_injection/jailbreak (not ATTACK_PATTERNS); allow telemetry keeps scan threat; routing stamps org_routing_enabled; FE flags unenforced injection. LIVE zs-1b34a6759ecd HTTP 400 block, model skip, Scan Detail 339285 BLOCK/95/HIGH/injection true. VERIFY: 46 GW + 44 FE + runs/scan-251434/.
 - PIPELINE-0034 (2026-09-17) — T01 PII-OFF TRUE BYPASS + AKIA SECRET: org toggle reaches scanner (not hardcoded True); SECRET-tagged PII stays secret when PII off; strip Tier-2 PII-only labels. LIVE zs-5dcb5ad52554 allow/none, zs-4e9333afc7b1 secret. VERIFY: 25 GW T01 tests + L01 API/Playwright + quality-signed.json.
 - PIPELINE-0028 (2026-07-06) — OUTPUT SMART-MASK PARITY + REASONING PROMOTION + ROUTING PIN: main.py _output_redact_redaction_possible (PIPELINE-0012 output parity on both redact paths) + _maybe_promote_reasoning_to_content (reasoning_content→content when blank, GATEWAY_PROMOTE_REASONING_TO_CONTENT default on); pipeline_trace policy hint for pre-masked prompts; liveGateway pinnedModelRoutingPreferences + SSE reasoning_content; Attack/Isolation simulators enable_routing:false; ModelRoutingSimulator enable_routing:true explicit. +5 tests test_pipeline_pre_masked_redact.py. Gate: gateway pytest.
...
<!-- perf-ralph --> PERF-0014 (2026-08-12) | Full-VM parallel local + ECR builds. Makefile exports nproc/BuildKit/COMPOSE_PARALLEL_LIMIT; `infra/scripts/vm-capacity.sh` + parallel `build-push-images.sh` (7 images via aim-fast buildx) + parallel `build-prod-images-local.sh`; frontend npm uses all cores. Platform stays linux/arm64 (c8g). AFFECTS: build tooling. ACTION: `make capacity`; `make ecr-push TAG=latest && make sync-ec2-deploy`.
...
<!-- mcp-page-cleanup --> MCP-PAGE-CLEANUP-23 (FREEZE gate: A–G re-verified 3×) | scripts/ralph/mcp_page_cleanup_item23_freeze.sh + login-retry-on-429 in mcp_page_typesim.mjs / item16_verify.py / item21_verify.py | The regression FREEZE: all section gates (A clean-errors=item21, B stuck-state=item09, C alignment/responsive SNAPSHOT GATE=item12, D redactions=item16 api+ui, E banner=item17, F simulator=item19, G every-tab=item22) run 3× back-to-back and must all be green. HARNESS FIX (not a product regression): the first freeze run tripped the control login throttle (login_user: 5/min per email — auth/throttling.py) because every gate signs in as the same admin and E alone opens 3 browser contexts → a 6th login got HTTP 429, cascading FAILs while the stack was healthy. FIXED by making the shared login helpers honor 429 (wait Retry-After / backoff, retry — production-correct) and spacing the orchestrator (14s gate gap, 45s round gap). | VERIFY: ITEM-23-FREEZE: PASS — 24/24 gates green across 3 rounds (R1/R2/R3 all PASS for A_cleanerr, B_stuckstate, C_snapshot, D1_redact_api, D2_redact_ui, E_banner, F_simulator, G_everytab). Section H (freeze, item 23) COMPLETE → the §1.4 Context Assembly & MCP page cleanup (items 00-23) is DONE.
 ...
 - CHG-0154 (2026-07-06) — MCP_HOST_TOOLS end-to-end wiring (on-demand sandbox CLI binaries): shared/ai_mesh_shared/mcp_host_tools.py + presets host_tools→env_vars.MCP_HOST_TOOLS + control serializer validation + MCP_HOST_TOOL_FAILED classifier + frontend Host CLI tools UI; agent stdio_manager uses shared contract. VERIFY: 22 host-tool unit tests + gateway 2091 + broker 196. Evidence mcp-parallel/findings/backstop-p-host-tools-wiring/.
 - CHG-0155 (2026-07-07) — Sandbox writable HOME + npx isEntrypoint spawn fix for MCP_HOST_TOOLS: HOME=/var/cache/home (semgrep ~/.semgrep on RO rootfs); stdio_manager _resolve_npx_spawn rewrites bare npx -y <pkg> → node <main.js> (mcp-server-semgrep isEntrypoint gate). VERIFY: live hostToolsQuickPass semgrep connected tools=7 + bogus MCP_HOST_TOOL_FAILED. Evidence mcp-parallel/findings/backstop-p-host-tools-writable-home/.
