...
- [x] 11. Fix the Tier-1 false positive: plain-text PII must NOT be classified as "Markdown/HTML-obfuscated"; obfuscation detection only fires on actual obfuscation. (Fix L3.) DONE: ROOT CAUSE = all four detect_* functions (detect_pii, detect_secrets, detect_credential_exposure, detect_ip_leakage) internally canonicalize (strip ZWC, fold fullwidth), so the G33/G53/G76 plain-text filters used canonicalized "raw" detections — (a) plain SSN near **bold** wrongly classified as obfuscated, (b) ZWC-interleaved credential wrongly filtered OUT. FIX: use _*_core variants (no canonicalization) for the filter in scanner.py (G33+G53) + mcp_scan_orchestrator.py (G76). 17 new tests in test_pipeline_obfuscation_fp.py. Gate: 17 targeted + 2000 full suite passed. PIPELINE-0011.
