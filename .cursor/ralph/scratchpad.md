...
- [ ] 03. Define ONE canonical pipeline with ONE enforcement authority (a single resolve+enforce function all paths call). Document in docs/pipeline/CANONICAL.md.
- [ ] 04. Refactor all paths to the canonical enforcement (minimal main.py edits; put logic in enforcement.py). Streaming + non-streaming + degraded all go through it.
...
- [ ] 09. Policy matches PII/PCI/PHI and REDACTS before input_scan (fix B-POL: compile+push to POLICY_SYNC, verify regex). (Fix L4.)
- [ ] 10. resolve_enforcement: REDACT recommendation → REDACT; escalate to block only if org policy=block or redaction byte-impossible. PII record → redact + forward masked. (Fix L5.)
