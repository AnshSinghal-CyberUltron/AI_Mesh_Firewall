...
---
## CHG-0136 (2026-07-03) — opt-in broker→agent auth (X-Sandbox-Agent-Key): 2nd cross-tenant isolation layer

- **Gap:** the in-sandbox agent's /rpc verified NOTHING (no key/identity) — Docker network isolation was the SOLE control preventing one sandbox from calling another org's agent. That single control can fail: (a) a host-run broker places sandboxes on the SHARED bridge (docker_manager._run_kwargs uses self.config.network when no broker container ref) where every sandbox reaches every other's agent port; (b) a per-org-network misconfig / Docker bug. Then an unauthenticated /rpc is cross-tenant-callable (invoke another org's MCP servers / read its data). Verified: production uses per-org networks (isolated); reconcile_registry attributes by LABEL_ORG_SLUG (sound); _build_child_env uses an allowlist so a spawned server can't read the agent env.
- **Fix:** opt-in MCP_AGENT_INTERNAL_KEY end-to-end. (1) agent main.py: /rpc requires a matching X-Sandbox-Agent-Key (constant-time hmac.compare_digest) when the env is set, else allow (backward-compat); (2) broker _post_agent_rpc (routes.py): attaches the header when configured; headers=_headers or None preserves the no-header contract (CHG-0121 tests still pass); (3) docker_manager _run_kwargs: provisions the SAME key into the sandbox env; (4) shared _SECRET_ENV_DENYLIST += MCP_AGENT_INTERNAL_KEY (and it's not in _SAFE_ENV_PASSTHROUGH) so _build_child_env NEVER passes it to a spawned MCP server — the key point that makes a single broker-wide key a real cross-tenant control.
- **Gate:** full broker suite 166 passed (2 lifecycle + 2 agent-ready-retry new); full agent suite green (agent auth test: missing/wrong->-32001, correct->dispatch, unset->allow); denylist byte-probe (_build_child_env refuses the key).
- **Evidence:** mcp-parallel/findings/backstop-p-agent-rpc-auth/finding.md. NOTE (host-run shared bridge is the concrete network-topology gap; a proper hard network lockdown remains infra). Promise WITHHELD (G5 live stress items 14-19 host-blocked).

---
## CHG-0137 (2026-07-03) — don't retry tools/call after dispatch (double-execution guard; retries/partial-failure)

- **Gap:** gateway/ai_mesh_gateway/mcp_sandbox_client.py broker_send_rpc forwards every JSON-RPC exchange via _request_with_503_retry, which retried on 503 (safe, pre-execution) AND on ANY httpx.HTTPError. httpx.HTTPError includes ReadTimeout: for a tools/call (gateway→broker→agent→upstream EXECUTES the tool then replies), a slow reply that ReadTimeouts on the gateway means the tool ALREADY ran, but the retry re-POSTs → the upstream executes the tool a SECOND time. tools/call is non-idempotent (send email/charge/delete/create) → duplicate side effect on a routine slow call or a chaos partial-failure. Found auditing the gateway→broker forwarding.
- **Fix:** idempotency-aware retry. _NON_IDEMPOTENT_METHODS={"tools/call"}; _request_with_503_retry(idempotent=True default) — on httpx.HTTPError, a NON-idempotent call retries ONLY if the error is PRE-SEND (_err_is_pre_send: ConnectError/ConnectTimeout/PoolTimeout — connection never established, tool couldn't have run); a POST-send error (ReadTimeout/reset) on a non-idempotent call fails immediately (RuntimeError, not retried). 503 retry unchanged (always pre-execution). broker_send_rpc passes idempotent=(method not in _NON_IDEMPOTENT_METHODS). Default True preserves ensure_sandbox behavior.
- **Gate:** test_mcp_sandbox_client.py 17 passed (4 new: tools/call+ReadTimeout NOT retried; tools/list+ReadTimeout retried; tools/call+ConnectError pre-send retried; tools/call+503 retried); full gateway suite 1874 passed 0 failed.
- **Evidence:** mcp-parallel/findings/backstop-p-toolscall-retry-double-exec/finding.md. Promise WITHHELD (G5 live stress items 14-19 host-blocked).

---
## CHG-0138 (2026-07-03) — regression-lock: agent SSRF classifier (IPv6 + IPv4-mapped-IPv6 + internal ranges)

- **Item:** rigorous-verification + lock of the agent SSRF egress guard. Test-only, no code change.
- **Verified sound:** _resolved_ip_blocked (CHG-0067) blocks _METADATA_IPS (169.254.169.254, fd00:ec2::254) + ipaddress is_private/loopback/link_local/reserved/multicast/unspecified; fail-closed on unparseable; _assert_upstream_not_ssrf checks ALL resolved IPs (DNS-rebind across A/AAAA). Byte-probed the classic IPv4-mapped-IPv6 bypass (::ffff:169.254.169.254, ::ffff:10.0.0.1) + full IPv6 battery -> ALL blocked (Python 3.14 ipaddress flags the mapped form is_private=True); public IPv4/IPv6 allowed. No bypass.
- **Gap closed:** existing SSRF tests (test_upstream_proxy.py) cover only the FLOW (internal host -> denied), NOT the classifier's IPv6/mapped/ULA/metadata/reserved coverage -> a refactor (or a Python version mis-handling mapped IPv6) could silently re-open SSRF egress with a green suite. Added test_ssrf_ip_classifier.py: parametrized must-block battery + must-allow public + 2 async end-to-end (_assert_upstream_not_ssrf on literal mapped-IPv6 metadata -> denied; public literal -> allowed).
- **Gate:** tests/test_ssrf_ip_classifier.py 23 passed; agent collect clean 107; broker suite 166 passed.
- **Residual (out of scope, documented):** DNS-rebinding TOCTOU — the guard resolves+validates then httpx re-resolves at connect; a low-TTL attacker domain could rebind to an internal IP between check and connect. A clean fix needs connection-level IP pinning httpx doesn't easily support. Evidence mcp-parallel/findings/backstop-p-ssrf-classifier-lock/finding.md. Promise WITHHELD (G5 stress items 14-19 host-blocked).

---
## CHG-0139 (2026-07-03) — regression-lock: MCP audit record carries NO raw PII/secret value (1.4 leak-at-rest)

- **Item:** rigorous-verification + lock of the audit leak-at-rest invariant. Test-only, no code change.
- **Verified sound:** _record_gateway_event serializes decision/policy_reason/compliance_tags/scan_findings(McpFinding.to_finding_dict)/metadata into the MCPEvent. McpFinding fields = entity_type/score/start-end OFFSETS/direction/tier/threat_type/detail(=TYPE names)/matched_kinds(TYPE names) — NO raw-value field. compliance_tags = tag CODES (normalized to catalog vocab at control-plane tasks.py _normalize_compliance_tags -> to_catalog_codes). metadata = {transport, enforced_at, scan_trace}. Byte-probed AWS key/email/SSN/GitHub PAT/Stripe/internal-IP: the raw value is ABSENT from json.dumps({findings, compliance_tags, metadata}) for every class; the redacted egress result also drops it.
- **Gap closed:** the redaction tests assert the EGRESS RESULT bytes drop the secret; NO test asserted the AUDIT RECORD bytes drop it (a distinct surface). A future McpFinding.detail that quoted the matched span (for debugging), or a caller threading raw content into metadata, would silently leak the exact secret/PII into the persisted MCPEvent (leak-at-rest) with a green suite. Added test_mcp_audit_no_raw_leak.py (parametrized battery: raw absent from findings/tags/metadata + scanner detected + result redacted).
- **Gate:** test_mcp_audit_no_raw_leak.py 6 passed; full gateway suite 1926 passed 0 failed.
- **Evidence:** mcp-parallel/findings/backstop-p-audit-no-raw-leak-lock/finding.md. Promise WITHHELD (G5 live stress items 14-19 host-blocked; item-21 UI cross-plane).

---
## CHG-0140 (2026-07-03) — internal chat-pipeline MCP routes: apply the inbound body-size cap (DoS parity)

- **Gap:** every MCP entry point should cap its inbound body (_mcp_body_too_large Content-Length pre-check CHG-0034 + _mcp_read_body_capped streaming cap CHG-0063; the _mcp_body_too_large docstring says the streaming cap is used by "every MCP entry point"). But gateway/ai_mesh_gateway/mcp_proxy.py internal_tools_call (/v1/mcp/internal/tools-call) and internal_discover_tools (/v1/mcp/internal/discover-tools) did `body = await request.json()` directly with NO cap -> the whole body buffers unbounded. org_mcp_jsonrpc/org_mcp_tool_call/ext_mcp_proxy all cap. The internal routes are called by the trusted backend BUT the payload carries the CHAT USER's tool ARGUMENTS (attacker-influenced), so a huge body -> gateway memory-exhaustion DoS.
- **Fix:** both internal routes now apply `if _mcp_body_too_large(request): return 413` + `try: await _mcp_read_body_capped(request) except _MCPBodyTooLarge: return 413`, after the internal-key check and before request.json() (reuses the cached capped bytes). Legit under-ceiling bodies unchanged.
- **Gate:** test_mcp_body_cap.py 13 passed (4 new, parametrized over both routes: oversized Content-Length -> 413 pre-check; chunked no-Content-Length over ceiling -> 413 streaming cap); full gateway suite 1930 passed 0 failed. All 5 MCP routes now apply _mcp_read_body_capped (grep-verified).
- **Evidence:** mcp-parallel/findings/backstop-p-internal-routes-body-cap/finding.md. Promise WITHHELD (G5 stress items 14-19 host-blocked; item-21 UI cross-plane).

---
## CHG-0141 (2026-07-03) — stdio transport default was fail-OPEN (untrusted npm/uvx spawned IN the gateway host); flip to secure-by-default (sandbox)

- **Item:** HARDEN THE ARCHITECTURE — "ALL transports in the per-org sandbox; NOTHING in the main backend; no unknown npm on the host." A fail-OPEN default on a security-critical isolation toggle.
- **Gap:** gateway/ai_mesh_gateway/mcp_stdio_adapter.py `_STDIO_IN_PROCESS_DEFAULT` was "true". `send_jsonrpc()` (L768) dispatches on `_stdio_in_process()` (L101): False -> `_send_jsonrpc_broker` (route the stdio server into the per-org gVisor sandbox via the broker); True -> `_send_jsonrpc_in_process` (spawn npx/uvx/node IN the gateway process, on the host). With the default "true", ANY deploy that did not explicitly set MCP_STDIO_IN_PROCESS=false ran org-registered stdio MCP servers ON THE GATEWAY HOST — untrusted 3rd-party npm/PyPI code executing in the shared multi-tenant process with cross-tenant process/fs sharing and no sandbox. That directly contradicts "ALL transports in the sandbox / no unknown npm on the host / NOTHING in the main backend". Fail-OPEN: the UNSAFE mode was the one you got by omission (a forgotten env var in any helm/k8s/bare deploy path, or a dev config promoted to prod).
...
---
## P6 — Pipeline transparency + UI (Cursor Ralph pipeline lane)
