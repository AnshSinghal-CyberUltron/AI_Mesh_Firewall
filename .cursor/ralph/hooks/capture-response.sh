...
# Check for <promise>TEXT</promise> in the response — ONLY when it is the
# final line (avoids false positives when the agent mentions the tag in prose).
PROMISE_TEXT=$(echo "$RESPONSE_TEXT" | perl -0777 -pe 's/.*?<promise>(.*?)<\/promise>.*/$1/s; s/^\s+|\s+$//g; s/\s+/ /g' 2>/dev/null || echo "")

if [[ -n "$PROMISE_TEXT" ]] && [[ "$PROMISE_TEXT" = "$COMPLETION_PROMISE" ]]; then
  if echo "$RESPONSE_TEXT" | perl -0777 -ne 'exit 0 if /<promise>\Q'"$COMPLETION_PROMISE"'\E<\/promise>\s*\z/s; exit 1'; then
    touch "$DONE_FLAG"
  fi
fi
