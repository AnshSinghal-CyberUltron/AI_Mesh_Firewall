/**
 * Human-language input_scan stage summaries for operator UI.
 */
...
function hasTechnicalDetails(stage = {}) {
  return Boolean(
    stage.guard_reason
...
/**
 * @param {object} stage model_routing-shaped input_scan stage from pipeline_trace
...
export function summarizeInputScanStage(stage = {}) {
  ...
  if (action === "block" && isInjectionThreat(stage.threat_type)) {
    summary = threat
      ? `Policy may have masked sensitive fields first. Input scan then detected ${threat} in the remaining text and blocked the request.`
      : "Input scan detected a jailbreak or injection pattern in the post-policy text and blocked the request.";
  } else if (action === "block") {
    summary = threat
      ? `Input scan blocked the request due to ${threat}.`
      : "Input scan blocked the request.";
...
export { hasTechnicalDetails as inputScanHasTechnicalDetails };
