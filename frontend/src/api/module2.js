...
export function createModule2Api(fetchWithAuth) {
  ...
  return {
    ...
    getUebaSettings: (opts = {}) => get("/ueba/settings/", {}, opts),
    patchUebaSettings: (data) => mutate("/ueba/settings/", "PATCH", data),
    getUebaLearningKeys: (opts = {}) => get("/ueba/api-keys/learning/", {}, opts),
    patchUebaKeySettings: (keyId, data) => mutate(`/ueba/api-keys/${keyId}/settings/`, "PATCH", data),
    reassessUebaKey: (keyId, data = {}) => mutate(`/ueba/api-keys/${keyId}/reassess/`, "POST", data),
