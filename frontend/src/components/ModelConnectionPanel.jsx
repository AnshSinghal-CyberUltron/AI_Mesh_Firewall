...
export function ModelConnectionPanel({
  ...
  const openEditModal = (model) => {
    setFormData({
      ...
      compliance_tags: Array.isArray(model.compliance_tags)
        ? model.compliance_tags.map(canonicalComplianceTag).filter(Boolean)
        : [],
      risk_score: model.risk_score === undefined || model.risk_score === null ? "" : String(model.risk_score),
  ...
  const handleSave = async (e) => {
    ...
    setError(null);
    try {
      // Routing configuration is COMPULSORY when adding a model. A model saved on bare
      // defaults (cost 0, SLA 30000, priority 0, risk 0) scores identically to every
      // other default model, so the weighted scorer has nothing to rank on — every
      // candidate ties and the winner falls to catalogue order. Caught here so the
      // operator sees which field is missing instead of a generic backend 400.
      if (!editingModel) {
        const requiredRouting = [
          ["data_sensitivity_level", "Data Sensitivity"],
          ["cost_per_1k_input_tokens", "Cost / 1K Input Tokens"],
          ["latency_sla_ms", "Latency SLA"],
          ["routing_priority", "Routing Priority"],
          ["risk_score", "Risk Score"],
        ];
        const missing = requiredRouting
          .filter(([key]) => {
        ...
        if (missing.length) {
          setRoutingExpanded(true);
          setError(
            `Routing configuration is required when adding a model. Missing: ${missing.join(", ")}. ` +
            `Without it this model cannot be ranked against the others and routing becomes arbitrary.`
          );
          setSubmitting(false);
          return;
        }
      ...
      const payload = {
        ...
        compliance_tags: (formData.compliance_tags || []).map(canonicalComplianceTag).filter(Boolean),
      };
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-4 mt-4">
                ...
                >
                  ...
                  Routing Configuration
                  <span className={`text-[10px] font-normal ${editingModel ? "text-slate-500 dark:text-slate-400" : "text-rose-600 dark:text-rose-400"}`}>
                    {editingModel ? "(optional)" : "(required)"}
                  </span>
                </button>
                {routingExpanded && (
                  <div className="space-y-3">
                    ...
                    <div>
                      <span className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Compliance Frameworks</span>
                      <div role="group" aria-label="Compliance frameworks" className="flex flex-wrap gap-1.5">
                        {COMPLIANCE_FRAMEWORKS.map((fw) => {
                          const selected = (formData.compliance_tags || []).includes(fw.value);
                          return (
                            <button
                              key={fw.value}
                              type="button"
                              ...
                              onClick={() => setFormData({
                                ...formData,
                                compliance_tags: selected
                                  ? (formData.compliance_tags || []).filter((t) => t !== fw.value)
                                  : [...(formData.compliance_tags || []), fw.value],
                              })}
                              className={`px-2 py-1 rounded text-[11px] font-medium ring-1 transition-colors ${
                                selected
                                  ? "bg-teal-500/20 text-teal-700 dark:text-teal-300 ring-teal-500/50"
                      ...
                      <p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">
                        A request demanding these frameworks will only route to models carrying all of them.
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label htmlFor="mcp-risk-score" className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Risk Score (0-1)</label>
                        <input id="mcp-risk-score"
                          type="number"
                          min="0"
                          max="1"
                          step="0.01"
                          ...
                          className="text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-800 w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                        />
                        <p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">0 = safest. Drives the Risk Avoidance weight.</p>
                      </div>
                      <div>
