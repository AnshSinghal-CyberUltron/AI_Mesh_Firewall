...
export function RAGPipelineSimulator() {
  ...
  const handleSend = async () => {
    ...
    if (!apiKey.trim()) {
      setError("Gateway API key is required. Create one in the Gateway API Keys panel.");
      return;
    }
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 space-y-5 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h3 className="text-slate-900 dark:text-slate-100 font-semibold text-base">RAG Pipeline Attack Simulator</h3>
          <p className="text-slate-500 dark:text-slate-400 text-xs">Test stage-by-stage enforcement via /v1/rag/query — {RAG_ATTACK_SCENARIOS.length} scenarios</p>
        </div>
        <InfoTooltip text="Sends real requests to the RAG pipeline endpoint and displays the per-stage pipeline audit trail showing how each stage handled the request. Covers query rewrite, document chain-of-custody, ranker policy filtering, and generator context verification." />
      </div>

      {/* Config row */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <label className="text-xs text-slate-400 dark:text-slate-500 mb-1 block">Gateway URL</label>
          <input
            value={gatewayUrl}
        ...
        <div>
          <label className="text-xs text-slate-400 dark:text-slate-500 mb-1 block">API Key</label>
          <input
            value={apiKey}
